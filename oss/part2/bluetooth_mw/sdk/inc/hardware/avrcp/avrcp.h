...
#if defined(MTK_AVRCP_APP_SETTINGS) && (MTK_AVRCP_APP_SETTINGS == TRUE)
  virtual void SendAppSettingUpdate(bool setting_changed) = 0;
#endif
...
class MediaInterface {
 public:
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) // [MTWTV-5957] add address parameter
  ...
  virtual void GetCapabilitiesCallback(const RawAddress& address, std::vector<uint8_t> elements) = 0;
  virtual void BatteryStatusChanged(const RawAddress& address, uint8_t status) = 0;
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) // [MTWTV-5957] add address parameter
  virtual void SetVolume(const RawAddress& bdaddr, int8_t volume) = 0;
#else
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE) // [MTWTV-5819] To support battery change
  virtual bool GetCapabilities(const RawAddress& bdaddr, Capability cap_id) = 0;
#endif
 protected:
