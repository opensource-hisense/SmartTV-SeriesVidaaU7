...
/** M: support audio delay  @{ */
/** Callback for audio delay value from sink devic.
 *  Used only for the A2DP src interface.
 *  delay value unit 0.1ms
 */
typedef void (* btav_audio_delay_callback)(const RawAddress& bd_addr,
                                                  uint16_t delay);
/** @} */
...
typedef struct {
  ...
  btav_audio_delay_callback audio_delay_cb;
#endif
  /** @} */
...
typedef struct {
  ...
  /** M: support audio delay  @{ */
  void (*set_itpool)(uint8_t bitpool);
  bt_status_t (*codec_enable)( uint8_t codec_type, uint8_t enable);
  /** @} */

//#if defined(MTK_DUAL_AUDIO) && (MTK_DUAL_AUDIO == TRUE)
  void (*switch_left_right)(const RawAddress& left_bd_addr, const RawAddress& right_bd_addr);
  void (*set_channel_allocation_for_lrmode)(const RawAddress& bd_addr, int channel);
  void (*set_audiomode)(int dualAudioMode);
  void (*set_sbc_channel_mode)(int ch_mode);
//#endif
...
typedef struct {
  ...
  /** M: sink add support  @{ */
  bt_status_t (*set_active_device)(const RawAddress& bd_addr);
  void (*delay_value)(const RawAddress& bd_addr, uint16_t value);
  bt_status_t (*codec_enable)( uint8_t codec_type, uint8_t enable);
  /** @} */
