...
#define UTIL_BLE_SCANNER_TIMER_CODE_OK              (0)
#define UTIL_BLE_SCANNER_TIMER_CODE_FAIL            (-1)
#define UTIL_BLE_SCANNER_TIMER_CODE_INV_ARG         (-2)
#define UTIL_BLE_SCANNER_TIMER_CODE_NO_RESOURCE     (-3)

#define ble_scanner_time_before(a, b) ((a).tv_sec < (b).tv_sec || ((a).tv_sec == (b).tv_sec && (a).tv_nsec < (b).tv_nsec))
...
typedef struct _BLE_SCANNER_TIMER_ENTRY_T
{
    struct _BLE_SCANNER_TIMER_ENTRY_T *previous;
    struct _BLE_SCANNER_TIMER_ENTRY_T *next;
    struct timespec abstime;
    struct timespec interval;
    UTIL_BLE_SCANNER_TIMER_HANDLER pf_callback;
    void *cb_args;  //set by the timer starter, and also shall be freed by the caller
    uint32_t id;
    UTIL_BLE_SCANNER_TIMER_REPEAT_TYPE_T repeat;
} BLE_SCANNER_TIMER_ENTRY_T;
...
static BLE_SCANNER_TIMER_ENTRY_T *s_timer_list;
static UINT32 s_timer_res;

static void ble_scanner_timer_list_add(BLE_SCANNER_TIMER_ENTRY_T *pt_timer)
{
    if (s_timer_list != NULL)
    {
        BLE_SCANNER_TIMER_ENTRY_T *p = s_timer_list;
        if (ble_scanner_time_before(pt_timer->abstime, p->abstime))
        {
...
static void ble_scanner_timer_list_remove(BLE_SCANNER_TIMER_ENTRY_T *pt_timer)
{
...
static void *BleScannerTimerProc(void *arg)
{
    BLE_SCANNER_TIMER_ENTRY_T *pt_timer;
    struct timespec abstime;
    int ret;
    ...
    while (!s_timer_quit)
    {
        ...
        while (s_timer_list && ble_scanner_time_before(s_timer_list->abstime, abstime))
        {
            ...
            //VERIFY(pthread_mutex_lock(&s_timer_list_mutex) == 0);
            ble_scanner_timer_list_remove(pt_timer);
            ...
            if (pt_timer->interval.tv_sec != 0 || pt_timer->interval.tv_nsec != 0)
            {
                ...
                ble_scanner_timer_list_add(pt_timer);
            }
        }
        if (s_timer_list != NULL)
        {
...
INT32 util_ble_scanner_timer_start(UTIL_BLE_SCANNER_TIMER_T *pt_timer)
{
    BLE_SCANNER_TIMER_ENTRY_T *entry = NULL;
    struct timespec delay;

    if (pt_timer == NULL)
    {
        return UTIL_BLE_SCANNER_TIMER_CODE_INV_ARG;
    }
    else if (pt_timer->handle == NULL)  //the timer is not created yet, create it
    {
        if ((pt_timer->pf_handler == NULL) || (pt_timer->delay_ms == 0) ||
            ((pt_timer->repeat != UTIL_BLE_SCANNER_TIMER_REPEAT_TYPE_ONCE) && (pt_timer->repeat != UTIL_BLE_SCANNER_TIMER_REPEAT_TYPE_REPEAT)))
        {
            return UTIL_BLE_SCANNER_TIMER_CODE_INV_ARG;
        }
        //Allocate a timer entry for this timer
        entry = (BLE_SCANNER_TIMER_ENTRY_T *)calloc(1, sizeof(BLE_SCANNER_TIMER_ENTRY_T));
        if (entry == NULL)
        {
            return UTIL_BLE_SCANNER_TIMER_CODE_NO_RESOURCE;
        }
    ...
    {
        entry = (BLE_SCANNER_TIMER_ENTRY_T *)pt_timer->handle;
    }
    ...
    {
        ble_scanner_timer_list_remove(entry);
    ...
    if (entry->repeat == UTIL_BLE_SCANNER_TIMER_REPEAT_TYPE_ONCE)
    {
    ...
    VERIFY(pthread_mutex_lock(&s_timer_list_mutex) == 0);
    ble_scanner_timer_list_add(entry);
    VERIFY(pthread_cond_signal(&s_timer_list_cond) == 0);
    VERIFY(pthread_mutex_unlock(&s_timer_list_mutex) == 0);
    return UTIL_BLE_SCANNER_TIMER_CODE_OK;
}

INT32 util_ble_scanner_timer_stop(VOID * h_timer)
{
    BLE_SCANNER_TIMER_ENTRY_T *pt_timer;
    struct timespec abstime;
    ...
    if (NULL == h_timer)
    {
        return UTIL_BLE_SCANNER_TIMER_CODE_FAIL;
    }
    pt_timer = (BLE_SCANNER_TIMER_ENTRY_T *)(h_timer);
...
INT32 util_ble_scanner_timer_delete(VOID * h_timer)
{
    BLE_SCANNER_TIMER_ENTRY_T *pt_timer;
...
INT32 util_ble_scanner_timer_resume(VOID * h_timer)
{
    BLE_SCANNER_TIMER_ENTRY_T *pt_timer;
    struct timespec abstime;
...
INT32 util_ble_scanner_timer_init(VOID)
{
    struct timespec res;
    ...
    VERIFY(pthread_create(&s_timer_thread, &attr, BleScannerTimerProc, NULL) == 0);
    return UTIL_BLE_SCANNER_TIMER_CODE_OK;
}
