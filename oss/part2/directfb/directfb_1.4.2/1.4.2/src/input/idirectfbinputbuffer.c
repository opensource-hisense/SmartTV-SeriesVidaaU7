...
typedef struct {
	 ...
	 DFBWindowEventType            window_mask;
...
DFBResult
IDirectFBEventBuffer_SetWindowMask( IDirectFBEventBuffer *thiz,
                                              DFBWindowEventType    mask)
{
     DIRECT_INTERFACE_GET_DATA(IDirectFBEventBuffer)

     D_DEBUG_AT( IDFBEvBuf, "%s( %d ) \n", __FUNCTION__, mask);

     data->window_mask = mask;

     return DFB_OK;
}

/*Wrap  IDirectFBEventBuffer_xxxx to _IDirectFBEventBuffer_xxxx for safe call
*/
static DirectResult
_IDirectFBEventBuffer_AddRef( IDirectFBEventBuffer *thiz )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_AddRef(thiz);
...
static DirectResult
_IDirectFBEventBuffer_Release( IDirectFBEventBuffer *thiz )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_Release(thiz);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_Reset( IDirectFBEventBuffer *thiz )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_Reset(thiz);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_WaitForEvent( IDirectFBEventBuffer *thiz )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_WaitForEvent(thiz);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_WaitForEventWithTimeout( IDirectFBEventBuffer *thiz,
                                              unsigned int          seconds,
                                              unsigned int          milli_seconds )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_WaitForEventWithTimeout(thiz,seconds,milli_seconds);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_WakeUp( IDirectFBEventBuffer *thiz )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_WakeUp(thiz);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_GetEvent( IDirectFBEventBuffer *thiz,
                               DFBEvent             *event )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_GetEvent(thiz,event);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_PeekEvent( IDirectFBEventBuffer *thiz,
                                DFBEvent             *event )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_PeekEvent(thiz,event);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_HasEvent( IDirectFBEventBuffer *thiz )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_HasEvent(thiz);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_PostEvent( IDirectFBEventBuffer *thiz,
                                const DFBEvent       *event )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_PostEvent(thiz,event);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_CreateFileDescriptor( IDirectFBEventBuffer *thiz,
                                           int                  *ret_fd )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_CreateFileDescriptor(thiz,ret_fd);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_EnableStatistics( IDirectFBEventBuffer *thiz,
                                       DFBBoolean            enable )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_EnableStatistics(thiz,enable);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}

static DFBResult
_IDirectFBEventBuffer_GetStatistics( IDirectFBEventBuffer *thiz,
                                    DFBEventBufferStats  *ret_stats )
{
    DFBResult ret = DFB_OK;
    DFB_GLOBAL_LOCK(DFB_LOCK_FOR_EXEC);
    ret = IDirectFBEventBuffer_GetStatistics(thiz,ret_stats);
    DFB_GLOBAL_UNLOCK(DFB_LOCK_FOR_EXEC);
    return ret;
}
                                ...
                                void                      *filter_ctx )
{
     ...
     thiz->AddRef                  = _IDirectFBEventBuffer_AddRef;
     thiz->Release                 = _IDirectFBEventBuffer_Release;
     thiz->Reset                   = _IDirectFBEventBuffer_Reset;
     thiz->WaitForEvent            = _IDirectFBEventBuffer_WaitForEvent;
     thiz->WaitForEventWithTimeout = _IDirectFBEventBuffer_WaitForEventWithTimeout;
     thiz->GetEvent                = _IDirectFBEventBuffer_GetEvent;
     thiz->PeekEvent               = _IDirectFBEventBuffer_PeekEvent;
     thiz->HasEvent                = _IDirectFBEventBuffer_HasEvent;
     thiz->PostEvent               = _IDirectFBEventBuffer_PostEvent;
     thiz->WakeUp                  = _IDirectFBEventBuffer_WakeUp;
     thiz->CreateFileDescriptor    = _IDirectFBEventBuffer_CreateFileDescriptor;
     thiz->EnableStatistics        = _IDirectFBEventBuffer_EnableStatistics;
     thiz->GetStatistics           = _IDirectFBEventBuffer_GetStatistics;
