                ...
                void      *ctx )
{
     ...
     DFBFontCache *cache = object;
     DFBFontCacheType    type = cache->type;

     DFB_FONT_CACHE_ASSERT( cache );

     DirectResult ret = direct_map_remove( map, &type );
     if (ret != DR_OK) {
         printf("[DFB] destroy_caches, remove cache map failed\n");
     }

     ret = dfb_font_cache_destroy( cache );
     if (ret != DR_OK) {
         printf("[DFB] %s, %d : destroy cache failed\n",__FUNCTION__,__LINE__);
     }
...
dfb_font_cache_row_deinit( DFBFontCacheRow *row )
{
     ...
     direct_list_foreach_safe (glyph, next, row->glyphs) {
          ...
          glyph_hash = font->layers[glyph->layer].glyph_hash;

          if (glyph_hash) {
          /*ret =*/ direct_hash_remove( glyph_hash, glyph->index, &glyph->tilt_matrix, glyph->attrFlag );
          //FIXME: use D_ASSERT( ret == DFB_OK );
          }

          if (glyph->index < 128 &&
              glyph->tilt_matrix.xx == 0x10000L && glyph->tilt_matrix.xy == 0 &&
              glyph->tilt_matrix.yx == 0 && glyph->tilt_matrix.yy == 0x10000L &&
              glyph->attrFlag == 0x0)
...
dfb_font_dispose( CoreFont *font )
{
     ...
     // clear cache
     direct_map_iterate( font->manager->caches, destroy_caches, NULL );
     font->manager->num_rows = 0;
                         ...
                         unsigned long  attrFlag)
{
     ...
     if (index < 128 && font->layers[layer].glyph_data[index] && pmatrix->xx == 0x10000L && pmatrix->xy == 0 && pmatrix->yx == 0 && pmatrix->yy == 0x10000L && attrFlag == 0x0) {
     ...
     data = direct_hash_lookup( font->layers[layer].glyph_hash, index, pmatrix, attrFlag  );
     if (data) {
        // Fix Bug [0529671]: direct_hash_remove() at direct_list_foreach_safe() might remove hash element fail,
        // since attrFlag not match at locate_key(), but still free CoreGlyphData memory.
        //D_MAGIC_ASSERT( data, CoreGlyphData );
        if ( data->magic != D_MAGIC("CoreGlyphData"))
        {
            direct_hash_remove( font->layers[layer].glyph_hash, index, pmatrix, attrFlag);
        }
        else
        {
            D_MAGIC_ASSERT( data, CoreGlyphData );
     ...
     if(font->SetItalics)
          font->SetItalics(font, pmatrix);

     if(font->SetAttrFlag)
          font->SetAttrFlag(font, attrFlag);
     ...
     memcpy(&data->tilt_matrix, pmatrix, sizeof(DFBMatrix));
     ...
     //FIXME : Should we ready need to flush texture cache here?
     //dfb_gfxcard_flush_texture_cache(0,0);
...
out:
     ...
     direct_hash_insert( font->layers[layer].glyph_hash, index, data, pmatrix, attrFlag );

     if (index < 128 && pmatrix->xx == 0x10000L && pmatrix->xy == 0 && pmatrix->yx == 0 && pmatrix->yy == 0x10000L && attrFlag == 0x0)
             ...
             void          *ctx )
{
     ...
     direct_hash_remove( hash, key, &data->tilt_matrix, data->attrFlag );
