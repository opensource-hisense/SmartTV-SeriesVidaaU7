...
/**********************************************************************************************************************/
typedef struct _CURSOR_RECT_BORDER
{
     int i4_enable;
     int i4_x_left;
     int i4_x_right;
...
static CURSOR_RECT_BORDER t_cursor_border;
...
{
     Stack_Container    *stack_cntr = NULL, *next = NULL;
...
dfb_windowstack_create( CoreLayerContext *context )
{
     ...
     DFBConfigLayer *conf  = &dfb_config->layers[context->layer_id];
     ...
     stack->bg.mode        = conf->background.mode;;
     stack->bg.color_index = -1;
     stack->bg.bRelease_image = false;

     if (dfb_config->mst_layer_bfullupdate[context->layer_id] == true)
         stack->bforce_fullupdate = true;
     else
         stack->bforce_fullupdate = false;

     if(DLBM_COLOR == stack->bg.mode)
     {
        stack->bg.color = conf->background.color;
        stack->bg.color_index = conf->background.color_index;
     }
     ...
     if( context->layer_id == dfb_config->mst_inputevent_layer )//FIXME
     {
...
dfb_windowstack_cursor_enable( CoreDFB *core, CoreWindowStack *stack, bool enable )
{
     ...
     D_INFO("[DFB][%s %d] %s pid=%d\n", __FUNCTION__, __LINE__, enable ? "enable" : "disable", getpid());
     ...
     if (dfb_config->no_cursor || stack->cursor.enabled == enable) {
          dfb_windowstack_unlock( stack );
          D_INFO("[DFB][%s %d] dfb_config->no_cursor=%d stack->cursor.enabled=%d pid=%d\n", __FUNCTION__, __LINE__, dfb_config->no_cursor, stack->cursor.enabled, getpid());
          return DFB_OK;
     }
     ...
     if(enable)
     {
	     stack->cursor.flags |= CCUF_ENABLE;
		 stack->cursor.flags &= (~CCUF_DISABLE);
     }
	 else
	 {
	     stack->cursor.flags |= CCUF_DISABLE;
...
dfb_windowstack_cursor_set_opacity( CoreWindowStack *stack, u8 opacity )
{
     ...
     if (stack->cursor.opacity != opacity) {
		  ...
		  stack->cursor.flags |= CCUF_OPACITY;
          /* Notify WM. */
                                  ...
                                  int              hot_y )
{
     ...
     if (dfb_config->no_cursor) {
          D_INFO("[DFB][%s %d] no cursor pid=%d\n", __FUNCTION__, __LINE__, getpid());
          return DFB_OK;
     }
...
dfb_windowstack_set_force_fullupdate(CoreWindowStack *stack,bool bforceupdate)
{
    ...
    stack->bforce_fullupdate = bforceupdate;
                                            ...
                                            void       *ctx )
{
     ...
     {
          /*
          If true means we force background image vanished,
          and background image get back to the default.
          */
          if ( stack->bg.image == notification->surface         ||
               stack->bg.bRelease_image)
          {
...
finish:
     if (f) {
          int ret2 = fclose( f );
          if(ret2 != 0) {
               direct_log_printf( NULL, "[%s %d] fclose fail, ret2=%d\n", __FUNCTION__, __LINE__, ret2 );
               ret = DFB_FAILURE;
          }
     }
                       ...
                       int              height )
{
     ...
     stack->cursor.x       = dfb_get_cursor_clip_x(stack->width  / 2);
     stack->cursor.y       = dfb_get_cursor_clip_y(stack->height / 2);
     stack->cursor.hot.x   = 0;
     stack->cursor.hot.y   = 0;
...
void dfb_set_cursor_border(int i4_enable,int i4_x_left,int i4_x_right,int i4_y_left,int i4_y_right)
{
    t_cursor_border.i4_enable   = i4_enable;
    t_cursor_border.i4_x_left   = i4_x_left;
    t_cursor_border.i4_x_right  = i4_x_right;
    t_cursor_border.i4_y_left   = i4_y_left;
    t_cursor_border.i4_y_right  = i4_y_right;
    
    direct_log_printf( NULL, "dfb_set_cursor_border [%d,%d-%d,%d-%d]\n", 
                            t_cursor_border.i4_enable, 
                            t_cursor_border.i4_x_left,
                            t_cursor_border.i4_x_right,
                            t_cursor_border.i4_y_left,
                            t_cursor_border.i4_y_right);

    return;
}

int dfb_get_cursor_clip_x(int i4_x)
{
    int i4_pos=i4_x;

    if(1 != t_cursor_border.i4_enable)
    {
        return i4_pos;
    }
    
    if(i4_x < t_cursor_border.i4_x_left)
    {
        i4_pos = t_cursor_border.i4_x_left;
    }
    else if(i4_x > t_cursor_border.i4_x_right)
    {
        i4_pos = t_cursor_border.i4_x_right;
    }
    
    //direct_log_printf( NULL, "dfb_get_cursor_clip_x pid[%d][%d,%d]\n",getpid(), i4_x,i4_pos);
    
    return i4_pos;
}

int dfb_get_cursor_clip_y(int i4_y)
{
    int i4_pos=i4_y;
    ...
    if(i4_y < t_cursor_border.i4_y_left)
    {
        i4_pos = t_cursor_border.i4_y_left;
    }
    else if(i4_y > t_cursor_border.i4_y_right)
    {
        i4_pos = t_cursor_border.i4_y_right;
    }
    
    //direct_log_printf( NULL, "dfb_get_cursor_clip_y [%d,%d]\n", i4_y,i4_pos);
