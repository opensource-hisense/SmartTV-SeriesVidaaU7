...
{                                                                               \
    ...
    {                                                                           \
        direct_log_printf(NULL,"Assert(%s,%d)\n",__FUNCTION__,__LINE__);        \
        return -1;                                                             \
    }                                                                           \
    u4_size -=u4_skip;                                                          \
    pc_data += u4_skip;                                                         \
...
#define DFB_KEY_LAN_CHECK_SKIP(u4_lp,u4_len)                                    \
{                                                                               \
    if(u4_lp>=u4_len)                                                          \
    {                                                                           \
        direct_log_printf(NULL,"DFB_KEY_LAN Error(%s,%d)\n",__FUNCTION__,__LINE__);\
...
typedef struct {
     ...
     DFB_KEY_LAN_OBJ     t_key_map;
     char               *pt_key_lan_type;
     char               *pt_key_lan_path;
     int                 i4_key_lan_reload_fg;
     int                 i4_key_lan_support_altgr;
} DFBInputCoreShared;
...
typedef struct __DFB_LocalInputDeviceInfo {
     DirectLink link;

     DFBInputDeviceID id;
     IDirectFBInputDevice* interface;
}DFB_LocalInputDeviceInfo;
...
static DirectLink * local_devices = NULL;
static pthread_mutex_t local_device_handle_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static int make_id( DFBInputDeviceID prefered );
...
dfb_input_core_initialize( CoreDFB            *core,
                           ...
                           DFBInputCoreShared *shared )
{
     DFB_BOOT_GETTIME( DF_BOOT_INPUT_CORE_INIT, DF_MEASURE_START, DF_BOOT_LV4);
     ...
     core_input->pt_key_lan_type   = NULL;
     core_input->pt_key_lan_path   = NULL;
     core_input->i4_key_lan_reload_fg = 0x1;
	 core_input->i4_key_lan_support_altgr = 0x1;
     core_input->t_key_map.pool = dfb_core_shmpool( core_local->core );
     ...
     DFB_BOOT_GETTIME( DF_BOOT_INPUT_CORE_INIT, DF_MEASURE_END, DF_BOOT_LV4);
     return DFB_OK;
                     ...
                     DFBInputCoreShared *shared )
{
     ...
     for (i=0; i<core_input->num; i++) {
	   ...
	   device->shared = core_input->devices[i];
          device->shared->id = make_id( device->shared->device_info.prefered_id);
	   device->core = core;
                         ...
                         bool          emergency )
{
     ...
     dfb_input_remove_all_local_device_handles();
...
#define MTKIO_CHECK_DFBDISCARDKEY     _IOW('M', 0x42, unsigned int)
...
************************************************************************************************/
   ...
   if(0 != strncmp(device->driver->info.name, "DFB NetEvent Driver", strlen(device->driver->info.name))
	   || 0 != strncmp(device->driver->info.vendor, "Mediatek", strlen(device->driver->info.vendor)))

   {
	   MTK_USERKEY_T ntKey;
	   strncpy(ntKey.ucToken, device->driver->info.name, sizeof(ntKey.ucToken));
	   ...
	   {
		   ntKey.u4IsKeyPress = (event->type == DIET_KEYPRESS) ? (1) : (0);
		   ntKey.u4KeySymbol = event->key_symbol;
		   ntKey.u4IsKey = 1;
	   }
	   if(MTK_DISCARD == ioctl(dfb_fb, MTKIO_CHECK_DFBDISCARDKEY, &ntKey))
	   {
		   return;
	   }
   }
...
************************************************************************************************/
     ...
     if (!((device->shared->id==DIDTF_VIRTUAL)&&(event->device_id !=DIDTF_NONE)))
          event->device_id = device->shared->id;

     DBG_INPUT_MSG("  -> DFBInputDeviceID  %x\n",event->device_id);
     ...
     if ((event->device_id != DIDID_MOUSE) && (event->device_id != DIDID_JOYSTICK)) {
          printf("[DFB]  => (%02x) %s%s%s DeviceID=%x KEYCODE=%d KEYSYMBOL=0x%04x\n", event->type,
                 dfb_input_event_type_name( event->type ),
                 (event->flags & DIEF_FOLLOW) ? " [FOLLOW]" : "",
                 (event->flags & DIEF_REPEAT) ? " [REPEAT]" : "",
                 event->device_id,
                 (event->flags & DIEF_KEYCODE) ? event->key_code : -1,
                 (event->flags & DIEF_KEYSYMBOL) ? event->key_symbol : -1);
     }
                                 ...
                                 char                      *filename )
{
     ...
     return load_keymap( device, filename, map );
}
...
DFBResult
dfb_input_load_physical_device_keymap(CoreInputDevice *device, char *filename, int deviceposition)
{
    InputDeviceKeymap *map = 0;

    D_MAGIC_ASSERT( device, CoreInputDevice );
    if (filename == NULL || deviceposition < 0)
        return DFB_FAILURE;

    DBG_INPUT_MSG("[DFB] filename = %s, deviceposition = %d\n", filename, deviceposition );

    map = &device->shared->physical_keymap[deviceposition];

    if ( (map && map->entries) == NULL)
    {
        DBG_INPUT_MSG("[DFB] %s (%d), invalid  map or map->entries\n", __FUNCTION__, __LINE__);
        return DFB_INVARG;
    }
...
/* Every physical device has own keymap */
DFBResult
dfb_input_allocate_physical_device_keymap(DFBInputDeviceID id, int deviceposition,  char *filename)
{
     DFBResult ret = DFB_OK;
     CoreInputDevice *device;

     if (deviceposition == INVALID_DEVICE_INDEX)
        return DFB_INVARG;

     device = dfb_input_device_at (id);
     /* check device is valid */
     if (!device)
     {
        DBG_INPUT_MSG("[DFB] invalid arg device\n");
        return DFB_INVARG;
     }
     ...
     int                                    num_entries         = desc->max_keycode - desc->min_keycode + 1;
     int                                    i;
     FusionSHMPoolShared       *shm_pool        = dfb_core_shmpool( device->core );

     D_ASSERT(shm_pool);

     /* check shared is valid */
     if (!shared)
     {
        DBG_INPUT_MSG("[DFB] invalid arg shared\n");
        return DFB_INVARG;
     }

     entries = SHCALLOC( shm_pool, num_entries, sizeof(DFBInputDeviceKeymapEntry) );
     if (!entries) {
          return D_OOSHM();
     }

     memset(entries, 0, sizeof(DFBInputDeviceKeymapEntry) * num_entries);

     shared->physical_keymap[deviceposition].min_keycode = desc->min_keycode;
     shared->physical_keymap[deviceposition].max_keycode = desc->max_keycode;
     shared->physical_keymap[deviceposition].num_entries = num_entries;
     shared->physical_keymap[deviceposition].entries     = entries;

    /* It's possible that users load keymap before input device is plug-in.Need to check..  */
    if (filename)
    {
        DBG_INPUT_MSG("[DFB] keymap: %s has already loaded before input device is plug-in\n", filename);
        ret = dfb_input_load_physical_device_keymap(device, filename, deviceposition);

        if (ret)
            DBG_INPUT_MSG("[DFB] deviceposition = %d, load physical device keymap : %s is error, ret = %d\n", deviceposition, filename, ret);
    }

    DBG_INPUT_MSG("[DFB] allocate physical_device_keymap end...\n");
    return ret;
}
/* Remove physical device, free the keymap */
DFBResult
dfb_input_deallocate_physical_device_keymap(DFBInputDeviceID id, int deviceposition)
{
     CoreInputDevice *device;
     ...
     shm_pool = dfb_core_shmpool(device->core );
     D_ASSERT(shm_pool);

     if ( shared->physical_keymap[deviceposition].entries){
          DBG_INPUT_MSG("[DFB] Ready to free physical_device_keymap  idx=%d\n",deviceposition);
          SHFREE( shm_pool, shared->physical_keymap[deviceposition].entries);
          shared->physical_keymap[deviceposition].entries = NULL;
     }

     DBG_INPUT_MSG("[DFB] Free physical_device_keymap end...\n");
     return DFB_OK;
}
DFBInputDeviceKeymapEntry *
dfb_input_get_entry_from_sub_device_keymap(DFBInputDeviceID id, int deviceposition, int keycode)
{
     ...
     if (!map)
        return NULL;
     /* sanity check */
     if (keycode < map->min_keycode || keycode > map->max_keycode)
          return NULL;

     /* point to right array index */
     entry = &map->entries[keycode - map->min_keycode];
		  ...
		  case CIDC_DEVICE_IOCTL:
                {
                      ...
                      if (!driver)
                     {
                           ...
                           *ret_val =  DFB_DESTROYED;
                           break;
                      }
                     if (driver->funcs->IOCtl==NULL)
                     {
                            *ret_val = DFB_UNSUPPORTED;
                            break;
                     }
                     *ret_val = driver->funcs->IOCtl(device, device->driver_data, (InputDeviceIoctlData*)call_ptr);
                 }
                 break;
...
/* do ioctl between app and device */
DFBResult
dfb_input_device_ioctl( CoreInputDevice           *device,
                                                       InputDeviceIoctlData *param)
{
     ...
     int                ret;
     InputDeviceShared *shared;
     InputDeviceIoctlData *shm_ictrl_data;
     FusionSHMPoolShared *shm_pool;
     ...
     D_INFO( "DirectFB/Input: ioctl for '%s' [0x%02x]...\n",
             shared->device_info.desc.name, shared->id );

     shm_pool = dfb_core_shmpool_data(device->core );
     D_ASSERT(shm_pool);
     if (param->request == DFB_DEV_IOC_SET_SUB_DEVICE_KEYMAP)
     {
           int length = 0;
           shm_ictrl_data = SHMALLOC(shm_pool, sizeof(*param));
           if(!shm_ictrl_data)
               return DFB_NOSHAREDMEMORY;

           shm_ictrl_data->request = param->request;

           DBG_INPUT_MSG("[DFB] User input =>File path is: %s, device name is : %s\n", (char*)param->param_ptr[0], (char*)param->param_ptr[1]);

           length = strlen((char*)param->param_ptr[0]) + 1;
           shm_ictrl_data->param_ptr[0]= SHMALLOC(shm_pool, length);
           memcpy (shm_ictrl_data->param_ptr[0], param->param_ptr[0], length);

           length = strlen((char*)param->param_ptr[1]) + 1;
           shm_ictrl_data->param_ptr[1]= SHMALLOC(shm_pool, length);
           memcpy (shm_ictrl_data->param_ptr[1], param->param_ptr[1], length);
           if (fusion_call_execute( &shared->call, FCEF_NONE, CIDC_DEVICE_IOCTL, shm_ictrl_data, &ret ))
           {
               SHFREE(shm_pool, shm_ictrl_data);
               return DFB_FUSION;
           }

           DBG_INPUT_MSG ("[DFB] Free shm_ictrl_data...\n");
           SHFREE(shm_pool, shm_ictrl_data->param_ptr[0]);
           SHFREE(shm_pool, shm_ictrl_data->param_ptr[1]);
           SHFREE(shm_pool, shm_ictrl_data);
           DBG_INPUT_MSG ("[DFB] Free shm_ictrl_data done...\n");

     }
     else if  (param->request == DFB_DEV_IOC_SET_LINUX_INPUT_REPEAT_TIME)
     {
           int length = 0;
           shm_ictrl_data = SHMALLOC(shm_pool, sizeof(*param));
           if(!shm_ictrl_data)
           ...
           DBG_INPUT_MSG("[DFB] device is: %s, first_repeat_time = %d, repeat_time = %d\n", (char*)param->param_ptr[0], param->param[1], param->param[2]);

           /* device name */
           length = strlen((char*)param->param_ptr[0]) + 1;
           shm_ictrl_data->param_ptr[0]= SHMALLOC(shm_pool, length);
           strncpy(shm_ictrl_data->param_ptr[0], param->param_ptr[0], length);

           /* first repeat time */
           shm_ictrl_data->param[1] = param->param[1];

           /* repeat time */
           shm_ictrl_data->param[2] = param->param[2];
         ...
         if(!shm_ictrl_data)
            return DFB_NOSHAREDMEMORY;
         memcpy(shm_ictrl_data, param, sizeof(*shm_ictrl_data));
         if (fusion_call_execute( &shared->call, FCEF_NONE, CIDC_DEVICE_IOCTL, shm_ictrl_data, &ret ))
         {
                SHFREE(shm_pool, shm_ictrl_data);
                return DFB_FUSION;
         }
         memcpy(param, shm_ictrl_data, sizeof(*shm_ictrl_data));
         SHFREE(shm_pool, shm_ictrl_data);
    }
    return ret;
...
{
     DFBResult                  ret       = DFB_OK;
     FILE                      *file      = NULL;//Fix converity END
     ...
     DBG_INPUT_MSG("[DFB] %s (%s) :Input Device ID %d loading keymap %s\n",
           __FUNCTION__, __FILE__, device->shared->id , filename);
     ...
     while(1)
     {
          ...
          /* copy the entry to the map */
          map->entries[keycode - map->min_keycode] = entry;
     }
     fclose(file);
     return ret;
}
                   ...
                   DFBInputEventFlags  lookup )
{
     ...
     if (lookup & DIEF_KEYSYMBOL) {
          ...
          if (DFB_KEY_TYPE( entry->symbols[DIKSI_BASE] ) == DIKT_MODIFIER)
               key_symbol = entry->symbols[DIKSI_BASE];
          else
               key_symbol = entry->symbols[index];
          if(key_symbol != 0)
            event->key_symbol = key_symbol;
     }
...
fixup_key_event( CoreInputDevice *device, DFBInputEvent *event )
{
     ...
     if (device->shared->keymap.num_entries) {
          if (valid & DIEF_KEYCODE) {
               if(lookup_from_table( device, event, missing ))
               {
     ...
     if (missing & DIEF_KEYSYMBOL)
     {
          if(event->key_symbol == DIKI_UNKNOWN || event->key_symbol == DIKI_KEY_UNMAPPING)
...
{
    if(dfb_config->mst_fixed_mem_leak_patch_enable == false)
    {
        // do nothing
        return DFB_OK;
    }

    DFB_LocalInputDeviceInfo* local_device = D_CALLOC(1, sizeof(DFB_LocalInputDeviceInfo));
    if (!local_device) {
         D_OOM();
    ...
    local_device->id = id;
    local_device->interface = interface;

    pthread_mutex_lock ( &local_device_handle_mutex );

    //direct_list_append( &local_devices, &local_device->link );
    direct_list_append( &local_devices, local_device);

    D_INFO("[DFB] insert local device handle: id = %d (%s, %d)\n", id, __FUNCTION__, __LINE__);

    pthread_mutex_unlock ( &local_device_handle_mutex );
...
DFBResult dfb_input_find_local_device_handle (int id, IDirectFBInputDevice ** interface)
{
    if(dfb_config->mst_fixed_mem_leak_patch_enable == false)
    {
        // do nothing and always return not found
        return DFB_IDNOTFOUND;
    }

    DFB_LocalInputDeviceInfo * local_device = NULL;

    pthread_mutex_lock ( &local_device_handle_mutex );

    direct_list_foreach(local_device, local_devices)
    {
        if(local_device->id == id)
        {
            *interface = local_device->interface;

            pthread_mutex_unlock ( &local_device_handle_mutex );

            D_INFO("[DFB] found local device handle: id = %d (%s, %d)\n", id, __FUNCTION__, __LINE__);
...
DFBResult dfb_input_remove_local_device_handle (int id)
{
    ...
    DFB_LocalInputDeviceInfo * next_device = NULL;
    DFB_LocalInputDeviceInfo * local_device = NULL;

    pthread_mutex_lock ( &local_device_handle_mutex );

    direct_list_foreach_safe(local_device, next_device, local_devices){

        if( id == local_device->id)
        {
            direct_list_remove(&local_devices, &local_device->link);

            D_INFO("[DFB] remove local device handle: id = %d (%s, %d)\n", id, __FUNCTION__, __LINE__);
            break;
        }
     }
...
DFBResult dfb_input_remove_all_local_device_handles()
{
    if(dfb_config->mst_fixed_mem_leak_patch_enable == false)
    {
        // do nothing
    ...
    D_INFO("[DFB] remove all local device handles (%s, %d)\n", __FUNCTION__, __LINE__);
...
static unsigned int dfb_key_lan_skip_space(char *puc_str,unsigned int u4_len)
{
    ...
    {
        u4_lp++;
        DFB_KEY_LAN_CHECK_SKIP(u4_lp,u4_len);
    }
...
static unsigned int dfb_key_lan_skip_char(char *puc_str,unsigned int u4_len,char u1_skip)
{
    ...
    while( u1_skip != puc_str[u4_skip] )
    {
        ...
        DFB_KEY_LAN_CHECK_SKIP(u4_skip,u4_len);
    }
...
{
    unsigned int u4_skip =0, skip_len = 0;
    ...
    skip_len = dfb_key_lan_skip_space(puc_str+u4_skip,u4_len - u4_skip);
    if(skip_len > (UINT_MAX - u4_skip))
    {
        D_ERROR("[DFB] %s, %d overflow occurs\n",__FUNCTION__, __LINE__);
        return 0;
    }
    u4_skip += skip_len;
...
static unsigned int dfb_key_lan_alloc_entry_mem(DFB_KEY_LAN_OBJ *pt_this)
{
    ...
    {
        SHFREE(pt_this->pool, pt_this->t_map.entries);
    ...
    pt_this->t_map.entries = SHMALLOC(pt_this->pool,pt_this->t_map.num_entries*sizeof(DFBInputDeviceKeymapEntry));
    if(!pt_this->t_map.entries)
    {
        DFB_KEY_LAN_ASSERT(0);
    }
...
static unsigned int dfb_key_lan_action_map(char *puc_str,unsigned int u4_len,DFB_KEY_LAN_OBJ *pt_this)
{
    ...
    #ifdef DFB_KEY_LAN_LOG
    direct_log_printf(NULL,"dfb_key_lan_action_map[%d]\n",u4_lp); 
    #endif
...
static unsigned int dfb_key_lan_load_data(char *ps_type,char *pc_buf,unsigned int u4_len,DFB_KEY_LAN_OBJ *pt_this)
{
    unsigned int i4_ret     = (unsigned int)DFB_OK;
    ...
    while(DFB_KEY_RST_LAN_MAX != pt_this->e_sate)
    {
        switch(pt_this->e_sate)
        {
            ...
            case DFB_KEY_RST_LAN_TYPE:
                ...
                DFB_KEY_LAN_CHECK_SKIP(u4_skip,u4_size);
...
{
    return (core_input->t_key_map.e_sate == DFB_KEY_RST_LAN_INITED);
}
...
void dfb_key_lan_load_keymap_init(void)
{
    ...
    #endif
    memset(&core_input->t_key_map,0x0,sizeof(DFB_KEY_LAN_OBJ));
...
{
    if(core_input->t_key_map.t_map.entries)
    {
        SHFREE(core_input->t_key_map.pool, core_input->t_key_map.t_map.entries);
    }
...
DFBResult dfb_key_lan_lookup_entry( unsigned int u4_key_code,DFBInputDeviceKeymapEntry* pt_entry )
{
    ...
    for( i=0; i < core_input->t_key_map.t_map.num_entries; i++ ) 
    {
        if( u4_key_code == core_input->t_key_map.t_map.entries[i].code)
        {
            memcpy(pt_entry,&core_input->t_key_map.t_map.entries[i],sizeof(DFBInputDeviceKeymapEntry));
            return DR_OK;
        }
    }
...
unsigned int dfb_key_lan_get_modi( unsigned int u4_idx)
{
    ...
    return core_input->t_key_map.au4_modi[u4_idx];
}
...
unsigned int dfb_key_lan_get_modi_mask( unsigned int u4_idx)
{
    ...
    return core_input->t_key_map.au4_modi_mask[u4_idx];
}
...
unsigned int dfb_key_lan_lookup_modi_id( unsigned int u4_val)
{
    ...
    {
        if(u4_val == core_input->t_key_map.au4_modi[u4_idx])
        {
            #ifdef DFB_KEY_LAN_LOG
...
DFBResult dfb_key_lan_set_type(char * ps_type)
{
    ...
    if(core_input->pt_key_lan_type)
    {
        if(0x0 == memcmp(core_input->pt_key_lan_type,ps_type,(strlen(ps_type))))
        {
            return DFB_OK;
        }
    }
    ...
    if(core_input->pt_key_lan_type)
    {
        SHFREE(dfb_core_shmpool( core_local->core ), core_input->pt_key_lan_type);
        core_input->pt_key_lan_type = NULL;
    }

    core_input->pt_key_lan_type = SHMALLOC(dfb_core_shmpool( core_local->core ), strlen(ps_type)+4);
    if(!core_input->pt_key_lan_type)
    {
        return DFB_FAILURE;
    }
    
    memset(core_input->pt_key_lan_type,0x0,(strlen(ps_type)+4));
    memcpy(core_input->pt_key_lan_type,ps_type,(strlen(ps_type)));
    dfb_key_lan_set_reload_fg(true);
    
    #ifdef DFB_KEY_LAN_LOG
    direct_log_printf(NULL,"dfb_key_lan_set_type(%s)\n",core_input->pt_key_lan_type); 
    #endif
...
{
    core_input->i4_key_lan_reload_fg = i4_fg;
    return;
}

int dfb_key_lan_get_reload_fg(void)
{
    return core_input->i4_key_lan_reload_fg;
}

void dfb_key_lan_set_support_altgr (int i4_fg)
{
    core_input->i4_key_lan_support_altgr = i4_fg;
    return;
}

int dfb_key_lan_get_support_altgr(void)
{
    return core_input->i4_key_lan_support_altgr;
}


char* dfb_key_lan_get_type(void)
{
    return core_input->pt_key_lan_type;
}
...
DFBResult dfb_key_lan_set_path(char * ps_path)
{
    ...
    if(core_input->pt_key_lan_path)
    {
        if(0x0 == memcmp(core_input->pt_key_lan_path,ps_path,(strlen(ps_path))))
        {
            return DFB_OK;
        }
    }
    ...
    if(core_input->pt_key_lan_path)
    {
        SHFREE(dfb_core_shmpool( core_local->core ), core_input->pt_key_lan_path);
        core_input->pt_key_lan_path = NULL;
    }

    core_input->pt_key_lan_path = SHMALLOC(dfb_core_shmpool( core_local->core ), strlen(ps_path)+4);
    if(!core_input->pt_key_lan_path)
    {
        return DFB_FAILURE;
    }
    
    memset(core_input->pt_key_lan_path,0x0,(strlen(ps_path)+4));
    memcpy(core_input->pt_key_lan_path,ps_path,(strlen(ps_path)));
    dfb_key_lan_set_reload_fg(true);
    
    #ifdef DFB_KEY_LAN_LOG
    direct_log_printf(NULL,"dfb_key_lan_set_path(%s)\n",core_input->pt_key_lan_path); 
    #endif
...
{
    return core_input->pt_key_lan_path;
}
...
DFBResult dfb_key_lan_load_keymap(char *type,char *filename )
{
    ...
    char                       *pc_buf     = NULL;
    char                       *ps_str     = DFB_KEY_LAN_TYPE_DEFAULT;
	char                       *ps_cmp     = "thai";
    DFBResult                  i4_ret      = DFB_OK;

    direct_log_printf(NULL, "\n\n\n\n\n===================\ndfb_key_lan_load_keymap(%s,%s)\n===================\n\n\n\n\n", type, filename); 
    
    if(!filename)
    {
        dfb_key_lan_set_path(DFB_KEY_LAN_PATH_DEFAULT);

        filename = DFB_KEY_LAN_PATH_DEFAULT;
    }
    ...
    if(memcmp(type,ps_str,strlen(type))&&memcmp(type,ps_cmp,strlen(type)))
    {
        dfb_key_lan_set_support_altgr(true);
    }
    else
    {
        dfb_key_lan_set_support_altgr(false);
    }    
    /* open the file */
    file = fopen( filename, "r" );
    if( !file )
    ...
    do
    {
        ...
        if(DFB_OK != (unsigned int)dfb_key_lan_load_data(type,pc_buf,u4_file_len,&core_input->t_key_map))
        {
            i4_ret = DFB_FAILURE;
            break;
        }
