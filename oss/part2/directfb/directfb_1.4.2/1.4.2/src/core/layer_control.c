...
DFBResult
dfb_layer_set_hv_scale( CoreLayer *layer, int HScale,int VScale )
{
     const DisplayLayerFuncs *funcs;
     CoreLayerShared   *shared;
     CoreLayerContexts *contexts;
     ...
     return funcs->SetHVScale( layer, layer->driver_data,
                             layer->layer_data, region->region_data, HScale,  VScale);
}

DFBResult
dfb_layer_Set_GopDst(         DFBDisplayLayerID  id, 
                           DFBDisplayLayerGOPDST gop_dst)
{
    DFBResult ret;
    CoreLayer *layer = dfb_layer_at(id);
    ...
    ret = funcs->SetGOPDst(layer,layer->driver_data ,layer->layer_data, gop_dst);

    return ret;
}

DFBResult
dfb_layer_Set_ForceWrite(CoreLayer *layer,  bool  force_write)
{
    DFBResult ret;
    const DisplayLayerFuncs *funcs = layer->funcs;
    CoreLayerShared *shared = layer->shared;
    ...
    if(!funcs->SetForceWrite)
        return DFB_UNSUPPORTED;

    ret = funcs->SetForceWrite(layer,layer->driver_data ,layer->layer_data, force_write);

    return ret;
}

DFBDisplayLayerGOPDST
dfb_layer_Get_GopDst(DFBDisplayLayerID  id)
{
    if( id < 0 || id >= dfb_layer_num() )
    {
        printf("[DFB] DirectFBGetGOPDst parameter error, invalid id = %d \n",id);
        return DLGD_UNKNOWN;
    }
    ...
    if(layer == NULL)
    {
        printf("[DFB] DirectFBGetGOPDst error!!! Please invoke DirectFBInit, DirectFBCreate & GetDisplayLayer first.\n");
        return DLGD_UNKNOWN;
    }

    const DisplayLayerFuncs *funcs = layer->funcs;
    CoreLayerShared *shared = layer->shared;
    DFBDisplayLayerGOPDST gop_dst = DLGD_UNKNOWN;

    D_ASSERT( layer != NULL );
    D_ASSERT( funcs != NULL );

    if(funcs == NULL || !funcs->GetGOPDst)
    {
        if(funcs == NULL)
            printf("[DFB] DirectFBGetGOPDst error!!! funcs == NULL\n");
        else
            printf("[DFB] DirectFBGetGOPDst error!!! funcs->GetGOPDst == NULL\n");

        return DLGD_UNKNOWN;
    }

    funcs->GetGOPDst(layer,layer->driver_data ,layer->layer_data, &gop_dst);
...
DFBResult
dfb_layer_set_hv_mirror_enable( CoreLayer *layer, bool HEnable,bool VEnable )
{
     const DisplayLayerFuncs *funcs;
     ...
     if (!funcs->SetHVMirrorEnable)
          return DFB_UNSUPPORTED;

     return funcs->SetHVMirrorEnable( layer, layer->driver_data,
                             layer->layer_data, HEnable,  VEnable);
}

DFBResult
dfb_layer_Set_Disable_Boot_Logo()
{
    DFBResult ret;
    ...
    if(!funcs->SetBootLogoPatch) {
        D_INFO("[DFB][%s %d] SetBootLogoPatch UNSUPPORTED\n", __FUNCTION__, __LINE__);
        return DFB_UNSUPPORTED;
    }

    ret = funcs->SetBootLogoPatch(layer, layer->driver_data, layer->layer_data, 0);
...
DFBResult
dfb_layer_Set_Enable_Zoom_Mode(int layer_id, int x, int y ,int w, int h)
{
    DFBResult ret;
    CoreLayer *layer =NULL;
    const DisplayLayerFuncs *funcs = NULL;

    layer = dfb_layer_at(layer_id);
    funcs = layer->funcs;

    if (!funcs->ScaleSourceRectangle)
        return DFB_UNSUPPORTED;

    ret = funcs->ScaleSourceRectangle(layer,layer->layer_data,x, y, w, h);

    if(ret != DFB_OK)
    {
        D_ERROR("[DFB] %s : layer %d scale failed\n",__FUNCTION__,layer_id);
        return ret;
    }
    return DFB_OK;
}

DFBResult dfb_layer_Set_GOP_Output_Ratio(  CoreLayer *layer, bool bEnable )
{
    DFBResult ret;
    const DisplayLayerFuncs *funcs = NULL;
    ...
    if (!funcs->SetGOPOutputRatio) {
        printf("[DFB] %s, not support SetGOPOutputRatio!\n", __FUNCTION__);
        return DFB_UNSUPPORTED;
    }

    return funcs->SetGOPOutputRatio( layer, layer->driver_data, layer->layer_data, bEnable);
