                             ...
                             unsigned int            *ret_num )
{
     ...
     type = surface->type & ~(CSTF_INTERNAL | CSTF_EXTERNAL | CSTF_AFBC);

     
     MTDFB_LOG(DFB_LOG_LVL_3,"%s[0x%x,0x%x,0x%x,0x%x,0x%x] \n",__FUNCTION__,
        surface->type,
        buffer->policy,
        buffer->surface->config.caps,
        access,
        type);
     ...
     for (i=0; i<pool_count; i++) {
		  ...
		  if (D_FLAGS_ARE_SET( pool->desc.types, type&(CSTF_MIU0|CSTF_MIU1|CSTF_MIU2)))
          {
             miu_match = true;
			 D_DEBUG_AT( Core_SurfacePool,"miu_match pool->desc.types[0x%x]\n",pool->desc.types);
          }

          if (D_FLAGS_ARE_SET( pool->desc.access[accessor], access ) &&
              D_FLAGS_ARE_SET( pool->desc.types, type ) /*&&
              D_FLAGS_ARE_SET( pool->desc.scaps, surface->config.caps )*/
              )
          {
               const SurfacePoolFuncs *funcs;
              ...
              if(funcs->TestConfig)
              {//in testconfig, chunk list will been accessed so we need to protect it*/

                     /* BUGFIX for Mantis 0593809 to avoid OOM problem. To retry twice to let Master Process
                        to have chance to release memory.
                     */
                    int retrycount = 0;
RETRY:
                    ...
                    if(ret == DFB_OK)
                    {
                        ...
                        ret = funcs->TestConfig( pool, pool->data, get_local(pool),
                                        buffer, &surface->config );

                        fusion_skirmish_dismiss( &pool->lock );

                       if(dfb_config->mst_oom_retry  == 0)
                       {
                           D_INFO("[DFB] mst_oom_retry = 0, no retry!\n");
                       }
                       else if ( (ret != DFB_OK  && ret != DFB_UNSUPPORTED) && (retrycount++ < dfb_config->mst_oom_retry) )
                       {
                           usleep(1000*2);
                           goto RETRY;
                       }
     ...
     *ret_num = num;
	 ...
	 if ((num==0)&&(!miu_match))
     {
        if(type & CSTF_MIU0)
        {
            printf("\n need DFB memory in MIU0 ,but no DFB memoey in MIU0\n");
        }
        else if(type & CSTF_MIU1)
        {
            printf("\n need DFB memory in MIU1 ,but no DFB memoey in MIU1\n");
        }
        else if(type & CSTF_MIU2)
        {
            printf("\n need DFB memory in MIU2 ,but no DFB memoey in MIU2\n");
        }
     }
                            ...
                            CoreSurfaceAllocation  **ret_allocation )
{
	 ...
	 CoreSurfaceTypeFlags  first_try_type=CSTF_NONE;
	 CoreSurfaceTypeFlags second_try_type=CSTF_NONE;
     ...
     D_DEBUG_AT( Core_SurfacePool, " -> %dx%d %s - %s%s%s%s%s%s%s%s%s\n",
                 ...
                 (surface->type & CSTF_PREALLOCATED_IN_VIDEO) ? " PREALLOCATED_IN_VIDEO" : "" );
    ...
    {
        direct_log_printf(NULL, "  -> NEGOTIATION FAILED! (%s)\n", DirectFBErrorString( ret ) );
	...
	switch (buffer->policy) {
    case CSP_SYSTEMONLY:
        first_try_type = CSTF_INTERNAL;
        second_try_type = CSTF_NONE;
        break;

    case CSP_VIDEOONLY:
        first_try_type = CSTF_EXTERNAL;
        second_try_type = CSTF_NONE;
        break;
        
    case CSP_VIDEOLOW:
        first_try_type = CSTF_INTERNAL;
        second_try_type = CSTF_EXTERNAL;
        break;

    case CSP_VIDEOHIGH:
        first_try_type = CSTF_EXTERNAL;
        second_try_type = CSTF_INTERNAL;
        break;
    }
	 ...
	 if (num_pools == 0)
	 {
		printf("\ncan not find proper memory pool to allocate the memory allocation\n");
		return DFB_FAILURE;
	 }
	 ...
	 for (i=0; i<num_pools; i++) {
		 ...
		 D_MAGIC_ASSERT( pool, CoreSurfacePool );
		 if(D_FLAGS_ARE_SET( pool->desc.types, first_try_type ) )
	 ...
	 /* second try to do the allocation in one of the pools */
	 if(!allocation && second_try_type != CSTF_NONE)
	 {
		 for (i=0; i<num_pools; i++) {
			 CoreSurfacePool *pool = pools[i];
			 ...
			 D_MAGIC_ASSERT( pool, CoreSurfacePool );
			 if(D_FLAGS_ARE_SET( pool->desc.types, second_try_type ))
				 ret = dfb_surface_pool_allocate( pool, buffer, &allocation );
			 else
				 continue;
     ...
     if(dfb_config->mst_surface_memory_type == -1)  //use default setting.
     {
         if (!allocation) {
     ...
     if (!allocation) {
          ...
          if (dfb_config->mst_surface_memory_type == -1) //default
          	printf("\n[DFB][PID=%d] allocate allocation failed! maybe out of memory !!!!!!!\n", getpid());
          //return DFB_FAILURE;
                       ...
                       CoreSurfaceBufferLock *lock )
{
     ...
     if(funcs == NULL)
     {
        direct_log_printf( NULL, "Core/SurfacePool: function is NULL, fail!\n" );
        return DFB_FAILURE;
     }
