               ...
               CoreSurface            **ret_surface )
{
     ...
     do {
          ret = dfb_layer_region_set_configuration( region, &config, (CLRCF_ALL &(~(CLRCF_DEST|CLRCF_CUR_DEST))));
          if (ret) {
...
dfb_window_create_region(CoreWindow *window)
{
    ...
    config.width         = window->surface->config.size.w;
    config.height        = window->surface->config.size.h;
    config.format        = window->surface->config.format;
    config.colorspace    = window->surface->config.colorspace;
    config.options       = context->config.options & DLOP_FLICKER_FILTERING;
    config.source        = (DFBRectangle) { 0, 0, config.width, config.height };
    config.dest          = (DFBRectangle) { 0, 0, screen_width, screen_height };
    config.opacity       = window->config.opacity;
    config.alpha_ramp[0] = 0x00;
    config.alpha_ramp[1] = 0x55;
    config.alpha_ramp[2] = 0xaa;
    config.alpha_ramp[3] = 0xff;
    ...
    if (((context->config.options & DLOP_ALPHACHANNEL) ||
          (window->config.options & DWOP_ALPHACHANNEL)) && DFB_PIXELFORMAT_HAS_ALPHA(config.format))
          config.options |= DLOP_ALPHACHANNEL;

    config.options |= DLOP_OPACITY;

    config.surface_caps = window->surface->config.caps & (DSCAPS_INTERLACED |
                                           DSCAPS_SEPARATED  |
                                           DSCAPS_PREMULTIPLIED);
    ...
    ret = dfb_layer_region_set_surface( region, window->surface);
                   ...
                   CoreWindow                 **ret_window )
{
     ...
     DFBSurfaceColorSpace    colorspace;
     DFBWindowID             toplevel_id;
     DFBRgb2YuvMode          rgb2yuvMode = FULL_RANGE; /* GE Settging for YUV format (LIMITED_RANGE/FULL_RANGE) */
     ...
     if (desc->width > dfb_config->mst_ge_hw_limit || desc->height > dfb_config->mst_ge_hw_limit)
     {
         D_ERROR("[DFB] (%s line %d) desc->width = %d, desc->height = %d, mst_ge_hw_limit = %d",__FUNCTION__,__LINE__, desc->width, desc->height, dfb_config->mst_ge_hw_limit);
     ...
     if (pixelformat == DSPF_YUY2 ||
          pixelformat == DSPF_UYVY ||
          pixelformat == DSPF_YVYU ||
          pixelformat == DSPF_VYUY) {
         rgb2yuvMode = desc->rgb2yuvMode;
     }
     ...
     colorspace   = desc->colorspace;
               ...
               surface_caps = desc->surface_caps & (DSCAPS_INTERLACED    | DSCAPS_SEPARATED  |
                                                    ...
                                                    DSCAPS_VIDEOONLY|DSCAPS_VIDEOHIGH|DSCAPS_FOUCE_VIDEOONLY|DSCAPS_TRIPLE);
          /*
               if(surface_caps & DSCAPS_FROM_3DMM)
               {
          ...
          */     
          ...
          surface_caps = desc->surface_caps & (DSCAPS_INTERLACED    | DSCAPS_SEPARATED  |
                                               DSCAPS_PREMULTIPLIED | DSCAPS_DEPTH      |
                                               DSCAPS_STATIC_ALLOC  | DSCAPS_SYSTEMONLY |
                                               DSCAPS_VIDEOONLY|DSCAPS_FOUCE_VIDEOONLY|DSCAPS_TRIPLE);
#endif

     if(desc->surface_caps & DSCAPS_SECURE_MODE)
     ...
     /*if(DFB_SYSTEMONLY)*/
	 if(DFB_SYSTEMONLY && (!(surface_caps & DSCAPS_FOUCE_VIDEOONLY)))
     {
        surface_policy = CSP_SYSTEMONLY;
     } 
     else if ((surface_caps & DSCAPS_VIDEOONLY) ||
     ...
     // Patch for Nxxxxt Project. Only layer surface use video memory, other depend on mst_surface_memory_type.
     switch(dfb_config->mst_surface_memory_type){
         case 1: //system only
             surface_policy = CSP_SYSTEMONLY;
             break;
         case 2: // video only
     ...
     if(dfb_config->window_double_buffer == true)
     {
        caps |= DWCAPS_DOUBLEBUFFER;
        surface_caps |= DSCAPS_DOUBLE;
     }

     if(dfb_config->window_single_buffer == true)
     {
        caps &= (~DWCAPS_DOUBLEBUFFER);
        surface_caps &= (~DSCAPS_DOUBLE);
     }
     ...
     if (dfb_config->mst_use_system_memory_threshold != 0) //system only
     {
         ...
         pitch = DFB_BYTES_PER_LINE( pixelformat, desc->width );
         length = DFB_PLANE_MULTIPLY( pixelformat, desc->height * pitch );

         if (caps & DWCAPS_DOUBLEBUFFER)
             length = length * 2;

         D_INFO("[DFB] width: %d, height: %d  buffer length: %d \n",desc->width, desc->height, length);
     ...
     config.rgb2yuvMode = rgb2yuvMode;
     ...
     if (! (caps & (DWCAPS_INPUTONLY | DWCAPS_COLOR))) {
          if (context->config.buffermode == DLBM_WINDOWS) {
               ...
               if (!window->surface) {
                    ...
                    surface->config.rgb2yuvMode = window->config.rgb2yuvMode;
                    if (ret) {
                   ...
                   int           height )
{
     ...
     if (width > dfb_config->mst_ge_hw_limit || height > dfb_config->mst_ge_hw_limit)
     {
                        ...
                        u8          opacity )
{
     ...
     config.opacity = opacity;
      ...
      if (window->region) 
      {
           CoreLayerRegion *region =  window->region;
           region->config.opacity = opacity;
      }   
#endif       
