                    ...
                    CoreSurface             **ret_surface )
{
     ...
     if (config) {
          ...
          if (config->flags & CSCONF_FORMAT) {
               ...
               surface->config.rgb2yuvMode = config->rgb2yuvMode;
          }
          ...
          if (config->flags & CSCONF_PREALLOCATED) {
              ...
              type |= CSTF_PREALLOCATED;
               surface->config.caps |= (DSCAPS_SYSTEMONLY | DSCAPS_PREALLOCATED);
          }

          if (config->flags & CSCONF_PREALLOCATED_IN_VIDEO) {
               D_DEBUG_AT( Core_Surface, "  -> prealloc in video %p [%d]\n",
                           config->preallocated[0].addr,
                           config->preallocated[0].pitch );
               ...
               if((dfb_config->bUsingHWTLB)&&(!dfb_config->bPrealloc_map_tlb))
                   surface->config.caps |= (DSCAPS_SYSTEMONLY | DSCAPS_PREALLOCATED);
               else
                   surface->config.caps |= DSCAPS_PREALLOCATED;
          }
     }
     ...
     if(surface->config.caps == DSCAPS_NONE)
     {
     ...
     surface->is_api_locked = false;

     surface->is_new_allocate_before_locked = true;
                      ...
                      const CoreSurfaceConfig *config )
{
     ...
     if(mt_get_dbg_info(0x3)&0x200){ 												   
           D_INFO("%s config = 0x%x  %d   %d  %s  0x%x 0x%x %d\n",
     	   __FUNCTION__,config->flags,config->size.w,config->size.h,dfb_pixelformat_name(config->format),config->colorspace,config->caps,flash_config);
     }
#endif
     D_MAGIC_ASSERT( surface, CoreSurface );
     D_ASSERT( config != NULL );
#if defined(CC_PHX_CUST) || (defined(CC_S_PLATFORM)&&defined(CC_LMP_PLATFORM))
     if((flash_config)&&(config->flags == 0XF )&&(config->size.w == 1366 )&&(config->size.h == 768 )&&(config->format == DSPF_ARGB )&&(config->caps == 0X15 )){
          flash_config = 0;
          return DFB_OK;
     }
#endif

     if (surface->type & (CSTF_PREALLOCATED|CSTF_PREALLOCATED_IN_VIDEO))
          return DFB_UNSUPPORTED;

     if (config->flags & (CSCONF_PREALLOCATED|CSCONF_PREALLOCATED_IN_VIDEO))
          return DFB_UNSUPPORTED;
...
dfb_surface_destroy_buffers( CoreSurface *surface )
{
     ...
     if (surface->type & (CSTF_PREALLOCATED|CSTF_PREALLOCATED_IN_VIDEO))
     {
     	  direct_log_printf( NULL,"unrealize_region dfb_surface_destroy_buffers unsupported\n");
          return DFB_UNSUPPORTED;
     }

     if (fusion_skirmish_prevail( &surface->lock ))
     {
     	  direct_log_printf( NULL,"unrealize_region dfb_surface_destroy_buffers fusion\n");
          return DFB_FUSION;
     }
     /* Destroy the Surface Buffers. */
	 direct_log_printf( NULL,"[chenli]dfb_surface_destroy_buffers surface->num_buffers=%d\n", surface->num_buffers);
     for (i=0; i<surface->num_buffers; i++) {
          dfb_surface_buffer_destroy( surface->buffers[i] );
          surface->buffers[i] = NULL;
     }
...
DFBResult
dfb_surface_parallel_dump_buffer( CoreSurface           *surface,
                         CoreSurfaceBufferRole  role,
                         const char            *path,
                         const char            *prefix,
                         int                        reset)
{
     ...
     ret = dfb_surface_parallel_buffer_dump( buffer, path, prefix, reset);
