...
pthread_mutex_t  core_dfb_lock = PTHREAD_MUTEX_INITIALIZER;
...
dfb_core_create( CoreDFB **ret_core )
{
     ...
     DFB_BOOT_GETTIME( DF_BOOT_SYSTEM_LOOKUP, DF_MEASURE_START, DF_BOOT_LV3);
     ret = dfb_system_lookup();
     DFB_BOOT_GETTIME( DF_BOOT_SYSTEM_LOOKUP, DF_MEASURE_END, DF_BOOT_LV3);
     if (ret)
          goto error;
     ...
     DFB_BOOT_GETTIME( DF_BOOT_FUSION_ENTER, DF_MEASURE_START, DF_BOOT_LV3);
     ret = fusion_enter( dfb_config->session, DIRECTFB_CORE_ABI, FER_ANY, &core->world );
     DFB_BOOT_GETTIME( DF_BOOT_FUSION_ENTER, DF_MEASURE_END, DF_BOOT_LV3);
     if (ret)
          goto error;
     ...
     DFB_BOOT_GETTIME( DF_BOOT_FUSION_ARENA_ENTER, DF_MEASURE_START, DF_BOOT_LV3);
     ...
     DFB_BOOT_GETTIME( DF_BOOT_FUSION_ARENA_ENTER, DF_MEASURE_END, DF_BOOT_LV3);
...
dfb_core_deinit_check( void *ctx )
{
     if (core_dfb && core_dfb->refs) {
          ...
          /*
           fix mantis 1850220
           if we set emergency to false which means the process will exit until the fusion dispatch loop thread terminate.
           Howerver emergency was to true, the process will directly end and can't wait for thread terminate.
           It might cause race condition if the memory is unmapping while the thread is using this memory.
          */
...
dfb_core_initialize( CoreDFB *core )
{
     ...
     ret = fusion_shm_pool_create( core->world, "DirectFB Data Pool", fusion_config->mst_directfb_data_pool_size*1024*1024,
                                   fusion_config->debugshm, &shared->shmpool_data );

     D_INFO("[DFB]dfb_config->mst_directfb_data_pool_size :%d (MB)\n", fusion_config->mst_directfb_data_pool_size);
     if (ret)
          return ret;
                           ...
                           void        *ctx )
{
     ...
     ret = fusion_shm_pool_create( core->world, "DirectFB Main Pool", fusion_config->mst_directfb_main_pool_size*1024*1024,
                                   fusion_config->debugshm, &pool );

     D_INFO("[DFB]dfb_config->mst_directfb_main_pool_size :%d (MB)\n", fusion_config->mst_directfb_main_pool_size);
     if (ret)
          return ret;
