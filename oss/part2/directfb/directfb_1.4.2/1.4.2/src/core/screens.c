...
dfb_screen_core_initialize( CoreDFB             *core,
                            ...
                            DFBScreenCoreShared *shared )
{
     DFB_BOOT_GETTIME( DF_BOOT_SCREEN_CORE_INIT, DF_MEASURE_START, DF_BOOT_LV4);
     int                  i;
     DFBResult            ret;
     ...
     for (i=0; i<num_screens; i++) {
          ...
          ScreenFuncs          *funcs  = NULL;
          DFBScreenDescription  desc   = { .caps = DSCCAPS_NONE };
		  
		  funcs  = (ScreenFuncs*)screen->funcs;
     ...
     DFB_BOOT_GETTIME( DF_BOOT_SCREEN_CORE_INIT, DF_MEASURE_END, DF_BOOT_LV4);
     return DFB_OK;
}
