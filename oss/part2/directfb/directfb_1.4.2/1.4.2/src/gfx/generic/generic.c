...
extern int _B2RBufNum;
extern unsigned int _B2RBufPhyAddr[10];
extern unsigned int _B2RBufVirAddr[10];
extern unsigned int _B2RBufCPhyAddr[10];
#endif
...
bool gAcquire( CardState *state, DFBAccelerationMask accel )
{
     ...
     switch (gfxs->dst_format) {
		  ...
		  case DSPF_NV16:
		  	   {
				   int n;
				   for(n = 0; n < _B2RBufNum; n++)
				   {
					   if( state->dst.phys == _B2RBufPhyAddr[n])
					   {
						   break;
					   }
				   }
				   ...
				   if(n < _B2RBufNum)
				   {
					   gfxs->dst_org[1] = gfxs->dst_org[0] + _B2RBufCPhyAddr[n] - _B2RBufPhyAddr[n];
				   }
				   else
				   {
					   gfxs->dst_org[1] = gfxs->dst_org[0] + gfxs->dst_height * gfxs->dst_pitch;  
     ...
     if (DFB_BLITTING_FUNCTION( accel )) {
          ...
          switch (gfxs->src_format) {
               ...
               case DSPF_NV16:
               {
                    ...
                    for(n = 0; n < _B2RBufNum; n++)
                    {
                        if( state->src.phys == _B2RBufPhyAddr[n])
                        {
                            break;
                        }
                    }
                    ...
                    if(n < _B2RBufNum)
                    {
                        gfxs->src_org[1] = gfxs->src_org[0] + _B2RBufCPhyAddr[n] - _B2RBufPhyAddr[n];
                    }
                    else
                    {
                        gfxs->src_org[1] = gfxs->src_org[0] + gfxs->src_height * gfxs->src_pitch;	
     ...
     switch (accel) {
          ...
          case DFXL_FILLTRIANGLE:
               if (state->drawingflags & ~(DSDRAW_DST_COLORKEY | DSDRAW_SRC_PREMULTIPLY | DSDRAW_DST_PREMULTIPLY)) {
                    GenefxAccumulator Cacc = {0}, SCacc = {0};
