                      ...
                      DFBRectangle *srect, DFBRectangle *drect )
{
	...
	if (dfb_config->mst_enhance_stretchblit_precision && (srect->x >= 0 && srect->y >= 0))
	{
		// Fix the precision issue
		const DFBRectangle orig_src = *srect;
		const DFBRectangle orig_dst = *drect;

		const int x_offset = orig_dst.x < 0 ? abs(orig_dst.x) * (srect->w / (float)orig_dst.w) : 0;
		const int y_offset = orig_dst.y < 0 ? abs(orig_dst.y) * (srect->h / (float)orig_dst.h) : 0;


		if(orig_dst.x < 0)
		{
		   drect->x = 0;
		   drect->w = orig_dst.w - abs(orig_dst.x);
		...
		{
		   drect->y = 0;
		   drect->h = orig_dst.h - abs(orig_dst.y);
		...
		srect->x += x_offset;
		srect->y += y_offset;
		srect->w = drect->w * (orig_src.w / (float) orig_dst.w);
		srect->h = drect->h * (orig_src.h / (float) orig_dst.h);

	}
	else
	{
		// Native solution of DFB
     ...
     if (drect->y != orig_dst.y)
		 ...
		 if (drect->w != orig_dst.w) {
			  //srect->w should not be zero after clip
			  int new_w = (int) (srect->w * (drect->w / (float)orig_dst.w) );
			  if(new_w < 1 && true == dfb_config->mst_clip_stretchblit_width_high_nonzero_patch)
				 new_w = 1;
		 ...
		 if (drect->h != orig_dst.h) {
			  //srect->h should not be zero after clip
			  int new_h = (int) (srect->h * (drect->h / (float)orig_dst.h) );
			  if(new_h < 1 && true == dfb_config->mst_clip_stretchblit_width_high_nonzero_patch)
				 new_h = 1;
...
void 
dfb_rectangle_correct(DFBRectangle *orig, DFBRectangle *d_rect, DFBRectangle *s_rect)
{
     int i = 0, ratio = 0, j =0;
	 DFBRectangle d = *d_rect, s = *s_rect;
	 int d_step, s_step, pitch;

/*calculate the the mininum step*/
	 for(i = 1; i < 100; i++)
	 {
	      if((orig->w % (2 * i)) || (s.w % (2 * i)))
	      {
	           break;
	      }
	 }
...
/*calculate the x, y position*/
	 ...
	 d_rect->x = orig->x + d_step * 2 * ( j - 1 );
	 s_rect->x = s_rect->x + s_step * 2 * (j - 1 );

/*calculate the w, h position*/
	 for(j = 1; j < i; j++)
	 {
	      pitch = d_step * 2 * j + orig->x;
		  if((d.x + d.w ) <  pitch)
	 ...
	 d_rect->w = d_step * 2 * j + orig->x - d_rect->x ;
	 s_rect->w = s_step * 2 * j + s.x - s_rect->x;

	 d_rect->w = ((d_rect->w + d_rect->x) > (orig->w + orig->x) ) ? (orig->w + orig->x -  d_rect->x) : d_rect->w;
     s_rect->w = ((s_rect->w + s_rect->x) > (s.w + s.x) ) ? (s.w + s.x -  s_rect->x) : s_rect->w;
	 ...
	 d_rect->y = orig->y + d_step * 2 * ( j - 1 );
	 s_rect->y = s_rect->y + s_step * 2 * (j - 1 );
	 ...
	 d_rect->h = d_step * 2 * j + orig->y - d_rect->y;
	 s_rect->h = s_step * 2 * j + s.y - s_rect->y;

     d_rect->h = ((d_rect->h + d_rect->y) > (orig->h + orig->y) ) ? (orig->h + orig->y -  d_rect->y) : d_rect->h;
     s_rect->h = ((s_rect->h + s_rect->y) > (s.h + s.y) ) ? (s.h + s.y -  s_rect->y) : s_rect->h;
...
void
dfb_clip_stretchblit_2( const DFBRegion *clip,
                      DFBRectangle *srect, DFBRectangle *drect )
{
     DFBRectangle orig_dst = *drect;
     ...
     dfb_rectangle_correct(&orig_dst, drect, srect);
}
