...
dfb_gfx_copy_to( CoreSurface *source, CoreSurface *destination, const DFBRectangle *rect, int x, int y, bool from_back )
{
   ...
   #if defined(CC_OSD_3DSBS_ENABLE)
     copy_state.sbsflag        =  1; 
   #elif defined(CC_OSD_3DTBB_ENABLE)
...
static void
back_to_front_copy_gles2( CoreSurface *surface, const DFBRegion *region, DFBSurfaceBlittingFlags flags, int rotation)
{
     DFBRectangle rect;
     int          dx, dy;
     void *sharecontext = 0;
     ...
     dfb_gfxcard_blit_gles2_makecurrent( &btf_state, &sharecontext);
     dfb_gfxcard_blit_gles2( &rect, dx, dy, &btf_state );
     dfb_gfxcard_blit_gles2_finish( &btf_state, sharecontext );
...
void
dfb_back_to_back_copy( CoreSurface *surface, const DFBRegion *region )
{
     DFBRectangle rect;
     int          dx, dy;
     void *sharecontext = 0;
...
void
dfb_layer_up_scaling_copy( CoreSurface *temp_surface, CoreSurface *surface, const DFBRegion *region )
{
     DFBRectangle rect;
     int          dx, dy;
     void *sharecontext = 0;
...
//D_INFO("[DFB] %s, %d\n", __FUNCTION__, __LINE__);
     ...
     // swap buffer
     if ( temp_surface->num_buffers >= 2 )
     {
          int tmp = temp_surface->buffer_indices[CSBR_BACK];
          temp_surface->buffer_indices[CSBR_BACK] = temp_surface->buffer_indices[CSBR_FRONT];
          temp_surface->buffer_indices[CSBR_FRONT] = tmp;
     }

     //from : FP16 front buffer , to : ARGB back buffer.
     if (!btf_state_inited) {
          dfb_state_init( &btf_state, NULL );

          btf_state.from = CSBR_FRONT;
          btf_state.to   = CSBR_BACK;  //to ARGB8888 back buffer.
     ...
     btf_state.clip.x2       = surface->config.size.w - 1;
     btf_state.clip.y2       = surface->config.size.h - 1;
     btf_state.source        = temp_surface;
     btf_state.destination   = surface;

     btf_state.src_blend = btf_state.dst_blend = DSBF_UNKNOWN;
...
/*static void (*back2front_copy[])( CoreSurface *surface,
                                  const DFBRegion *region,
                                  DFBSurfaceBlittingFlags flags,
                                  int rotation) = {
     back_to_front_copy, 
     back_to_front_copy_gles2,
};*/

void
dfb_back_to_front_copy( CoreSurface *surface, const DFBRegion *region )
{
     //back2front_copy[dfb_config->mst_enable_GLES2]( surface, region, DSBLIT_NOFX, 0);
...
void
dfb_back_to_front_stretch_copy_with_clipRegion( CoreSurface *surface, const DFBRegion *regiondst,
                       const DFBRegion *regionsrc, DFBSurfaceBlittingFlags flags, const DFBRegion *clipRegion)
{
     DFBRectangle rectdst;
     DFBRectangle rectsrc;

     if (regiondst) {
          rectdst.x = regiondst->x1;
          rectdst.y = regiondst->y1;
          rectdst.w = regiondst->x2 - regiondst->x1 + 1;
          rectdst.h = regiondst->y2 - regiondst->y1 + 1;
     }
     else {
          rectdst.x = 0;
          rectdst.y = 0;
          rectdst.w = surface->config.size.w;
          rectdst.h = surface->config.size.h;
     }

      if (regionsrc) {
          rectsrc.x = regionsrc->x1;
          rectsrc.y = regionsrc->y1;
          rectsrc.w = regionsrc->x2 - regionsrc->x1 + 1;
          rectsrc.h = regionsrc->y2 - regionsrc->y1 + 1;
     }
     else {
          rectsrc.x = 0;
          rectsrc.y = 0;
          rectsrc.w = surface->config.size.w;
          rectsrc.h = surface->config.size.h;
     }
     ...
     btf_state.clip           = *clipRegion;

     if(btf_state.clip.x2 >= surface->config.size.w)
        btf_state.clip.x2 = surface->config.size.w - 1;

     if(btf_state.clip.y2 >= surface->config.size.h - 1)
        btf_state.clip.y2 = surface->config.size.h - 1;
     ...
     if ( dfb_config->mst_enable_layer_autoscaledown == true)
         dfb_gfxcard_stretchblit_inner( &rectsrc, &rectdst, &btf_state );
     else
         dfb_gfxcard_stretchblit( &rectsrc, &rectdst, &btf_state );
