    ...
    Chunk *bestFit = NULL;
     ...
     while(c)
     {
         if(c->buffer && c->length<=length)
         {
                ...
                if(!(surface->type&CSTF_LAYER) && !(surface->type&CSTF_WINDOW) && !(surface->config.caps & DSCAPS_STATIC_POS) && !(surface->is_api_locked) )
                {
...
{    
     ...
     printf("\n DFB OOM Note: coming into the fragment merge \n");
     ...
     while(c)
     {
        if(c->buffer&&c->next&&(NULL==c->next->buffer))
        {
           CoreSurfaceBuffer *buffer = c->buffer;
           CoreSurface *surface  = buffer->surface;
           if(!(surface->type&CSTF_LAYER) && !(surface->type&CSTF_WINDOW)  && !(surface->config.caps & DSCAPS_STATIC_POS) && (buffer->locked) && !(surface->is_api_locked) )
           {
                if(needretry)
                    *needretry = true;
           }
...
*/
           ...
           else if(c->next&&(NULL==c->next->buffer))
           {
             ...
             while(bestFit)
             {
               ...
               if(ret)
               {
                 D_INFO("warning: Try to lock the bestfit allocation surface failed\n");
                 break;
               }

               if(buffer->locked)
               {
                 D_INFO("warning: Try to lock the bestfit allocation locked\n");
                 dfb_surface_unlock( surface );
                 break;
               }
...
*/
    ...
    printf("\33[0;33;44m[DFB]fragment Merge\33[0m \n");
           ...
           while(c &&(c->buffer || c->length< length))
            {
                ...
                {                        
                    ...
                    {
                        printf("[dfb] =====>  1 c->offset = [%x]    c->length = [%x]    c->next->offset = [%x]\n",c->offset,c->length,c->next->offset); 
                        usleep(500000);
                        printf("[dfb] =====> 2 c->offset = [%x]    c->length = [%x]    c->next->offset = [%x]\n",c->offset,c->length,c->next->offset); 
                    }
                                       ...
                                       CoreSurfaceBuffer *buffer )
{
     ...
     dfb_gfxcard_calc_buffer_size( dfb_core_get_part( core, DFCP_GRAPHICS ), buffer, NULL, &length, DSCAPS_VIDEOONLY);
