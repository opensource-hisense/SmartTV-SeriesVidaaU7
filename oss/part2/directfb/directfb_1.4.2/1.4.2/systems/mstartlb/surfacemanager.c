...
//we only do the remap the KPage/DPage to the new TLBAddress,
//not do the memory move
...

{
     ...
     DevMstarTLBPoolLocalData *local  = pool_local;
     DevMstarTLBPoolSharedData  *shared = local->sharedData;
     ...
     while(c)
     {
        if(c->buffer&&c->next&&(NULL==c->next->buffer))
        {
           ...
           if(!(surface->type&CSTF_LAYER) && !(surface->config.caps & DSCAPS_STATIC_POS) && !(buffer->locked))
           {
              ...
              Chunk *next = c->next;
              void * source = c->offset;
              void * dest   = c->offset + next->length;
            ...
            if (c->allocation->accessed[CSAID_GPU] & (CSAF_WRITE|CSAF_READ)) {
              ...
              */
             {
                CoreSurfaceAllocation *allocation = c->allocation;
                DevMstarTLBAllocationData *alloc = allocation->data;
                MoveInfo transfer;
                transfer.handle = alloc->handle;
                transfer.dest = dest;
                HWTLB_TransferBuffer(devMstarTLB, &transfer);
              ...
              ((DevMstarTLBAllocationData *)c->allocation->data)->offset = c->offset;
           ...
           else if(c->next&&(NULL==c->next->buffer))
           {
                ...
                while(bestFit)
                {
                  ...
                  {
                    CoreSurfaceAllocation *allocation = bestFit->allocation;
                    DevMstarTLBAllocationData *alloc = allocation->data;
                    MoveInfo transfer;
                    transfer.handle = alloc->handle;
                    transfer.dest = dest_chunk->offset;
                    HWTLB_TransferBuffer(devMstarTLB, &transfer);
                  }
                   ...
                   ((DevMstarTLBAllocationData *)bestFit->allocation->data)->offset = bestFit->offset;
                           ...
                           SurfaceManager **ret_manager, TLBMemInfo* tlbMemInfo )
{
    ...
    manager->offset  = 0;
    manager->length  = tlbMemInfo->msttlb_videoMem_length ;
    manager->avail   = tlbMemInfo->msttlb_videoMem_length;
                                       ...
                                       Chunk                 **ret_chunk )
{
     ...
     if((prealloc_phyaddr)&&(prealloc_phyaddr%(dfb_config->TLBAlignmentSize)))
    {
        length += dfb_config->TLBAlignmentSize;
    }
