               ...
               CoreSurfacePoolDescription *ret_desc )
{
     ...
     ret_desc->types             = CSTF_DEDICATED | CSTF_LAYER | CSTF_SHARED | CSTF_EXTERNAL;
     ret_desc->priority          = CSPP_DEFAULT;
     ret_desc->size              = dfb_config->video_length;


     if (dfb_config->mst_miu2_cpu_offset != 0 && dfb_config->video_phys_cpu >= dfb_config->mst_miu2_cpu_offset)
         ret_desc->types |= CSTF_MIU2;
     else if (dfb_config->video_phys_cpu >= dfb_config->mst_miu1_cpu_offset)
         ret_desc->types |= CSTF_MIU1;
     else
         ret_desc->types |= CSTF_MIU0;
      ret_desc->types |= CSTF_DECLARABLE;

     DBG_SUR_MSG("Surface  %s(%d) types=0x%x \n",__FUNCTION__,__LINE__,ret_desc->types);
                 ...
                 const CoreSurfaceConfig *config )
{
     ...
     DBG_SUR_MSG( "DevMem_Surfaces,%s( %p )\n", __FUNCTION__, buffer );
      ...
      if((ret == DFB_NOVIDEOMEMORY) && (retrycount-- > 0) && needretry)
        {
          /* To reduce CPU loading when HRT is enabled, extend the sleep time from 1 ms to 5 ms.
              In HRT case, usleep(1000) causes a large scale CPU overhead.
          */
          usleep(5*1000);
        goto RETRY;
        }
                     ...
                     void                  *alloc_data )
{
     ...
     DBG_SUR_MSG( "DevMem_Surfaces,  -> offset %d, pitch %d, size %d\n", alloc->offset, alloc->pitch, alloc->size );
            ...
            CoreSurfaceBufferLock *lock )
{
     ...
     lock->phys   = dfb_config->video_phys_cpu + alloc->offset;
               ...
               CoreSurfacePoolDescription *ret_desc )
{
     ...
     if (dfb_config->mst_miu2_cpu_offset != 0 && dfb_config->video_phys_secondary_cpu >= dfb_config->mst_miu2_cpu_offset)
          ret_desc->types |= CSTF_MIU2;
     else if (dfb_config->video_phys_secondary_cpu >= dfb_config->mst_miu1_cpu_offset)
         ret_desc->types |= CSTF_MIU1;
     else
         ret_desc->types |= CSTF_MIU0;
...
RETRY:
      ...
      {
          /* To reduce CPU loading when HRT is enabled, extend the sleep time from 1 ms to 5 ms.
              In HRT case, usleep(1000) causes a large scale of CPU overhead.
          */
          usleep(5*1000);
        goto RETRY;
      }
            ...
            CoreSurfaceBufferLock *lock )
{
     ...
     lock->offset = alloc->offset;
     lock->addr   = local->mem + alloc->offset;
     lock->phys   = dfb_config->video_phys_secondary_cpu + alloc->offset;
