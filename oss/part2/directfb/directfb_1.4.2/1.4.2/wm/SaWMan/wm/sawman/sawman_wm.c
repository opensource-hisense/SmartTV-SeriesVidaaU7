                   ...
                   const DFBInputEvent *event )
{
     ...
     if (window->config.options & DWOP_SCALE && window->surface != NULL) {
...
#else
     sawwin = sawman->pointer_window ? sawman->pointer_window : sawman->focused_window;    
#endif
     if (sawwin)
                   ...
                   DFBWindowEventFlags  flags )
{
     ...
     SaWManTier *tier_0;
     sawman_tier_by_layer( sawman, DLID_PRIMARY, &tier_0 );      
     ...
     cx = (s64) x * (s64) tier->size.w / (s64) tier_0->size.w;
     cy = (s64) y * (s64) tier->size.h / (s64) tier_0->size.h;
               ...
               int              dy )
{
     ...
     if (flags & DWCF_TRAPPED) {
          ...
          x1 = (s64) x1 * (s64) tier_0->size.w / (s64) tier->size.w;
          y1 = (s64) y1 * (s64) tier_0->size.h / (s64) tier->size.h;
          x2 = (s64) x2 * (s64) tier_0->size.w / (s64) tier->size.w;
          y2 = (s64) y2 * (s64) tier_0->size.h / (s64) tier->size.h;
     ...
     if (flags & DWCF_UNCLIPPED) {
          ...
          if (cx < 0)
               cx = 0;
          else if (cx >= tier_0->size.w)
               cx = tier_0->size.w - 1;

          if (cy < 0)
               cy = 0;
          else if (cy >= tier_0->size.h)
               cy = tier_0->size.h - 1;
     ...
     if (flags & DWCF_FIXED)
          ...
          stack->cursor.x = dfb_get_cursor_clip_x(cx);
          stack->cursor.y = dfb_get_cursor_clip_y(cy);
          stack->cursor.flags |= CCUF_POSITION;
          ...
          switch (event->axis) {
               case DIAI_X:
                    if (event->flags & DIEF_MAX)
                         axisabs = (s64) axisabs * (s64) tier_0->size.w / (s64) (event->max - axismin + 1);
               ...
               case DIAI_Y:
                    if (event->flags & DIEF_MAX)
                         axisabs = (s64) axisabs * (s64) tier_0->size.h / (s64) (event->max - axismin + 1);
                  ...
                  const DFBInputEvent *event )
{
     ...
     // direct_log_printf(NULL,"%s[pid = %d][type=0x%x,key_code=%d] \n","wm_process_input()",getpid(),event->type,event->key_code);
                        ...
                        int                     y )
{
     ...
     x = (s64) x * (s64) tier_0->size.w / (s64) tier->size.w;
     y = (s64) y * (s64) tier_0->size.h / (s64) tier->size.h;

     D_DEBUG_AT( SaWMan_WM, "  => %d, %d\n", x, y );

     if (x < 0)
          x = 0;
     else if (x >= tier_0->size.w)
          x = tier_0->size.w - 1;

     if (y < 0)
          y = 0;
     else if (y >= tier_0->size.h)
          y = tier_0->size.h - 1;
     ...
     if (x != stack->cursor.x || y != stack->cursor.y) {
          ...
          stack->cursor.x = dfb_get_cursor_clip_x(x);
          stack->cursor.y = dfb_get_cursor_clip_y(y);
          stack->cursor.flags |= CCUF_POSITION;
          dfb_wm_update_cursor( stack, CCUF_POSITION );
     }
                 ...
                 DFBSurfaceFlipFlags  flags )
{
     ...
     if (stack->forse_update || (!tier->single_mode))
#else
     if (!tier->single_mode)
#endif	 	
	 {
               ...
               bool                 right_eye )
{
     ...
     for (i=0; i<num; i++) {
          ...
          if (!regions || dfb_rectangle_intersect_by_region( &tmp, &regions[i] )) {
               ...
               {
                   dfb_gfx_copy_to_ex( window->surface, tier->region->surface, &src, tmp.x, tmp.y, false, window->config.opacity); 
               }
               ...
               if (tier->cursor_drawn && dfb_region_region_intersect( &cursor_visible, &reg )) {
                    ...
                    x = (s64) stack->cursor.x * (s64) size.w / (s64) tier_0->size.w;
                    y = (s64) stack->cursor.y * (s64) size.h / (s64) tier_0->size.h;
     ...
     if (flags & DSFLIP_QUEUE) {
          ...
          if (tier->single_mode && tier->single_window != NULL) {
               if (tier->single_window == sawwin) {
...
#ifdef CC_DFB_DEBUG_SUPPORT						
					struct timeval _s_sync;
					struct timeval _e_sync;
					if(dfb_enable_dump(30))
					{
						 gettimeofday(&_s_sync, 0);
					}
#endif
                    bool need_sync = true;
                    if((left_region == NULL) || (tier->optimize_single && tier->last_update.x1 >= left_region->x1 
						                                               && tier->last_update.y1 >= left_region->y1 
						                                               && tier->last_update.x2 <= left_region->x2
						                                               && tier->last_update.y2 <= left_region->y2))
                    {
                        need_sync = false;
                    }
					...
					if(need_sync)
					{
					    sawman_sync_buffer(sawman, tier);
					}

					tier->optimize_single = false;

#ifdef CC_DFB_DEBUG_SUPPORT
				   if(dfb_enable_dump(30))
				   {
					   gettimeofday(&_e_sync, 0);

					   D_INFO("%s, sync buffer: %d \n",__FUNCTION__, 
							   (_e_sync.tv_sec - _s_sync.tv_sec) * 1000000 
								   + _e_sync.tv_usec - _s_sync.tv_usec);
				   }
#endif

#ifdef CC_DFB_DEBUG_SUPPORT						
					struct timeval _s_repaint;
					struct timeval _e_repaint;
					if(dfb_enable_dump(30))
					{
						 gettimeofday(&_s_repaint, 0);
					}
#endif
                    ...
                    if (flags & DSFLIP_FLUSH) {
                         ...
                         if (sawwin->left.updates.num_regions) {
                              ...
                              update_single( sawman, sawwin, tier, sawwin->left.updates.regions, sawwin->left.updates.num_regions, flags, false );
                         }
                         ...
                         if (stereo_layer) {
                              if (sawwin->right.updates.num_regions) {
                                   ...
                                   update_single( sawman, sawwin, tier, sawwin->right.updates.regions, sawwin->right.updates.num_regions, flags, true );
                              }
                         }
                    }
                    else {
                         if (left_region)
                         {
                              update_single( sawman, sawwin, tier, left_region, 1, flags, false );

							  if(left_region)
							  {
							      tier->last_update.x1 = left_region->x1;
								  tier->last_update.x2 = left_region->x2;
								  tier->last_update.y1 = left_region->y1;
								  tier->last_update.y2 = left_region->y2;
							  }
							  ...
							  {
							      tier->last_update.x1 = 0;
								  tier->last_update.y1 = 0;
								  tier->last_update.x2 = sawwin->src.w - 1;
								  tier->last_update.y2 = sawwin->src.h - 1;
							  }

							  tier->optimize_single = true;
                         }

                         if (stereo_layer && right_region)
                              update_single( sawman, sawwin, tier, right_region, 1, flags, true );
                    }
                    ...
                    if(dfb_enable_dump(30))
                    {
					     gettimeofday(&_e_repaint, 0);

					     D_INFO("%s, repaint: %d \n",__FUNCTION__,  
						       (_e_repaint.tv_sec - _s_repaint.tv_sec) * 1000000 
						       + _e_repaint.tv_usec - _s_repaint.tv_usec);
                    }
#endif					
               }
          }
                  ...
                  CoreCursorUpdateFlags  flags )
{
     ...
     x = (s64) stack->cursor.x * (s64) mx / (s64) tier_0->size.w;
     y = (s64) stack->cursor.y * (s64) my / (s64) tier_0->size.h;
     ...
     if (flags & CCUF_DISABLE) {
		  ...
		  dfb_layer_region_lock( sawman->cursor.region );	  
          if (sawman->cursor.region->surface) 
		  {
               dfb_surface_detach_global( sawman->cursor.region->surface,
                                          &sawman->cursor.region->surface_reaction );
               dfb_surface_unlink( &sawman->cursor.region->surface );
          }
          dfb_layer_region_unlock( sawman->cursor.region );		  

		  sawman->cursor.region->surface = NULL;
     ...
     if (flags & CCUF_OPACITY) {
          ...
          config_flags |= CLRCF_OPACITY | CLRCF_OPTIONS;
     ...
     if (flags & CCUF_SHAPE && stack->cursor.enabled) {
                  ...
                  CoreCursorUpdateFlags  flags )
{
     ...
     if (!tier->cursor_bs) {
          ...
          ret = dfb_surface_create_simple(  wmdata->core, 
                                                stack->cursor.size.w, 
                                                stack->cursor.size.h,
                                                context->config.pixelformat, 
                                                DSCAPS_NONE,
                                                context->config.colorspace,
                                                CSTF_SHARED | CSTF_CURSOR, 
                                                (unsigned long)0, 
                                                NULL, 
                                                &cursor_bs );
          if (ret) {
