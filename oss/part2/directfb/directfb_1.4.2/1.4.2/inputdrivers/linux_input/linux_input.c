...
udev_hotplug_EventThread(DirectThread *thread, void * hotplug_data)
{
     ...
     if (socket_fd == -1) {
          DBG_INFO( "DirectFB/linux_input: socket() failed: %s\n",
                    strerror(errno) );
          goto errorExit;
     }
    ...
    if(bind(socket_fd, (struct sockaddr *) &addr, sizeof(addr)) < 0)
    {
     ...
     while(1) {
          ...
          *pos = '\0';
          ...
          if (!strcmp(udev_event, "add")) {
               DBG_INFO(  "Device node /dev/input/event%d is created by udev\n",
                           device_num);
               ...
               if ( DFB_OK == ret) {
                     ...
                     */
                    if ( DFB_OK != ret) {
                         DBG_INFO(   "Linux/Input: Failed to create the "
               ...
               DBG_INFO(   "Device node /dev/input/event%d is removed by udev\n",
                           device_num );
               ...
               if ( DFB_OK == ret) {
                     ...
                     */
                    if ( DFB_OK != ret) {
                         DBG_INFO(   "Linux/Input: Failed to remove the "
                                     "device for /dev/input/event%d\n",
                                     device_num );
                    }
               }
     ...
     DBG_INFO(  "Finished hotplug detection thread within Linux Input provider.\n" );
     return NULL;

errorExit:
     DBG_INFO( "Linux/Input: Fail to open udev socket, disable detecting "
