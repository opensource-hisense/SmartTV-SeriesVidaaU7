...
#define CC_ANDROID_TWO_WORLDS 1
...
#ifndef LINUX_TMP_PATH
#define LINUX_TMP_PATH "/data/vendor/tmp"
#endif
			   ...
			   if(( recvfd >= 0 ) && ( FD_ISSET(recvfd, &fdset) ))
			   {
					...
					if((num = read(recvfd, &inputdata,sizeof(DFBSocketEvent)) > 0))
					{
						 ...
						 if(inputdata.m_Type == 0)//KEY 
						 {
							 ...
							 else if(inputdata.u.Key.m_Type == 2)//PRESS+RELEASE try to fix socket delay issue
							 {
