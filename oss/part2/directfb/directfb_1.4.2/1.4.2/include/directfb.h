...
#define DFB_LINUX_INPUT_SET_SUB_DEVICE_KEYMAP   14
#define DFB_LINUX_INPUT_SET_REPEAT_TIME 15
...
#define DFB_DEV_IOC_SET_SUB_DEVICE_KEYMAP                  _IOW(MDRV_DFB_IOC_MAGIC,   DFB_LINUX_INPUT_SET_SUB_DEVICE_KEYMAP, void*)
#define DFB_DEV_IOC_SET_LINUX_INPUT_REPEAT_TIME                  _IOW(MDRV_DFB_IOC_MAGIC,   DFB_LINUX_INPUT_SET_REPEAT_TIME, void*)
...
DFBResult DirectFBGetMIUCPUOffset(
                                              MIUIdentifier identifer,
                                              unsigned long *puloffset
                                              );

DFBResult DirectFBGetMIUHALOffset(
                                              MIUIdentifier identifier,
                                              unsigned long * puloffset
                                              );

u64 DirectFBHalAddrToBusAddr(
                                              u64 halPhysicalAddr
                                              );
...
DFBResult DirectFBDisableBootLogo( void );
DFBResult DirectFBEnableZoommode(int layer_id, int x, int y ,int w, int h);
...
typedef enum {
     ...
     DSCAPS_IOMMU         = 0x00002000,  /* DFB use iommu. */

     DSCAPS_AFBC          = 0x00004000,  /* support GOP AFBC */

     DSCAPS_SECURE_MODE   = 0x00008000, /* secure mode */
     ...
     DSCAPS_SHARED        = 0x00100000,  /* The surface will be accessible among processes. */
       ...
       DSCAPS_GPU_CHECK   = 0x00200000,    /* for GPU to check surface if never be locked */
     ...
     DSCAPS_ALL          = 0xFF33FFF7,  /* All of these. */
...
typedef enum {
     ...
     DWOP_MULTI_VIEW     = 0x00800000,  /* Resize and MoveTo don't trigger repaint_stack. */
     DWOP_ALL            = 0x00F3707F   /* all possible options */
} DFBWindowOptions;
...
typedef enum {
     ...
     DFDESC_BOLD_STRENGTH     = 0x00000400,  /* bold strength is set */
...
typedef enum {
     ...
     DSPF_LUT1      = DFB_SURFACE_PIXELFORMAT( 40,  1, 0, 1, 1, 0, 7, 0, 0, 1, 0 ),

     /* 16 bit   YUV (4 byte/ 2 pixel, macropixel contains CbYCrY [31:0]) */
     DSPF_YVYU      = DFB_SURFACE_PIXELFORMAT( 41, 16, 0, 0, 0, 2, 0, 0, 0, 0, 0 ),

     /* 16 bit  BLINK12355 (2 byte, 1 1@15, NA 2@13, Blink 2@11, F/B 1@10, FG 5@5, BG 5@0) */
     DSPF_BLINK12355 = DFB_SURFACE_PIXELFORMAT( 42, 16, 0, 1, 0, 2, 0, 0, 0, 0, 0 ),

     /* 16 bit  BLINK12355 (2 byte, FA 2@14, BA 2@12, FG 6@6, BG 6@0) */
     DSPF_BLINK2266  = DFB_SURFACE_PIXELFORMAT( 43, 16, 0, 1, 0, 2, 0, 0, 0, 0, 0 ),

     /* 16 bit   YUV (4 byte/ 2 pixel, macropixel contains YCrYCb [31:0]) */
     DSPF_VYUY = DFB_SURFACE_PIXELFORMAT( 44, 16, 0, 0, 0, 2, 0, 0, 0, 0, 0 ),

     DSPF_ARGB2101010 = DFB_SURFACE_PIXELFORMAT( 45, 30, 2, 0, 0, 4, 0, 0, 0, 0, 0 ),

     DSPF_ABGR2101010 = DFB_SURFACE_PIXELFORMAT( 46, 30, 2, 0, 0, 4, 0, 0, 0, 0, 0 ),
...
#define DFB_COLOR_IS_RGB(fmt)           \
     (((fmt) == DSPF_ARGB1555)     ||   \
      ...
      ((fmt) == DSPF_BGR555)       ||   \
      ((fmt) == DSPF_RGBAF88871) || \
      ((fmt) == DSPF_ARGB2101010) || \
      ((fmt) == DSPF_ABGR2101010))
...
typedef enum {
     ...
     DSLT_NONE                = 0x00000000,       /* none of these */
     DSLT_IOVA                = 0x00000001        /* ret_phys is IOVA address */

}DFBSurfaceLock4RetType;

/*
 * Lock4 structure
 */
typedef struct __attribute__((packed)){
     size_t                             size;       /* Input sizeof(Lock4Parameter) from Caller */

     void                               *ret_ptr;   /* Virtual address */
     u64                                ret_phys;   /* Physical address(64bits MIU > 4G) */
     int                                ret_pitch;  /* Line patch */
     int                                ret_fd;     /* Dmabuf_fd */
     DFBSurfaceLock4RetType             ret_type;   /* Judge some type */
     unsigned int                       ret_offset; /* Offset */
} Lock4Parameter;

/*
 * GE setting for YUV format
 */
typedef enum {
     LIMITED_RANGE    = 0x00000000, //Y's values are shifted and scaled to the range [16, 235]
     FULL_RANGE       = 0x00000001, //Y's values are shifted and scaled to full range [0, 255]
} DFBRgb2YuvMode;
...
typedef struct {
     ...
     DFBRgb2YuvMode                      rgb2yuvMode; /* GE Settging for YUV format (LIMITED_RANGE/FULL_RANGE) */
...
/*
 * Called for each existing input device.
 * can be used to get device name.
 */
typedef DFBEnumerationResult (* DFBGetSubDeviceNameCallback) (
     DFBInputDeviceID  device_id,
...
D_DEFINE_INTERFACE(   IDirectFBDisplayLayer,
   ...
   /** Windows **/
    ...
    DFBResult (*SetHVMirrorEnable)(IDirectFBDisplayLayer         *thiz,
                                            ...
                                            bool                   VEnable);
      /*
    API  enable GOP output ratio
      */
    DFBResult (*SetGOPOutputRatio)(IDirectFBDisplayLayer         *thiz,
                                            bool                   bEnable);
)
...
typedef enum {
     ...
     DSTF_BACKGROUND    = 0x00000080,  /*use the blend function set by userspace*/
...
D_DEFINE_INTERFACE(   IDirectFBSurface,
   ...
   /** Blitting control **/
    ...
    DFBResult (*SetBlitNearestMode) (
      ...
      * Lock the surface for the access type specified.
      *
      * Returns a virtual address  and physical address(64bits MIU > 4G) the line pitch of it.<br>
      * <br>
      * <b>Note:</b> If the surface is double/triple buffered and
      * the DSLF_WRITE flag is specified, the pointer is to the back
      * buffer.  In all other cases, the pointer is to the front
...
typedef enum {
     ...
     DIEF_NOREPEAT       = 0x1000,   /* bypass repeated event, e.g. key or button press */
     DIEF_MIN            = 0x2000,   /* minimum value is set, e.g. for absolute axis motion */
     DIEF_MAX            = 0x4000,   /* maximum value is set, e.g. for absolute axis motion */
} DFBInputEventFlags;
...
typedef enum {
     ...
     DWEF_NOREPEAT       = 0x00000004,  /* no-repeat event, e.g. repeating key */
