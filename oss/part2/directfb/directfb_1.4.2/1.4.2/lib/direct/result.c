...
DirectResultTypeRegister( DirectResultType *type )
{
    ...
    attrFlag  = 0x0;
     ...
     if (direct_hash_lookup( &result_types, type->base, &matrix, attrFlag  )) {
          D_ASSERT( direct_hash_lookup( &result_types, type->base, &matrix, attrFlag ) == type );
          ...
          ret = direct_hash_insert( &result_types, type->base, (void*) type, &matrix, attrFlag );
          if (ret)
               D_MAGIC_CLEAR( type );
          else
               type->refs = 1;
...
DirectResultTypeUnregister( DirectResultType *type )
{
     ...
     if (! --(type->refs)) {
          ...
          ret = direct_hash_remove( &result_types, type->base, &matrix, attrFlag );
     }
...
DirectResultString( DirectResult result )
{
     ...
     if (result) {
          ...
          type = direct_hash_lookup( &result_types, D_RESULT_TYPE( result ), &matrix, attrFlag  );
