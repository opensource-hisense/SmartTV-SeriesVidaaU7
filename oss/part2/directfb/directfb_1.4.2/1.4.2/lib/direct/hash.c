...
locate_key( const DirectHash *hash, unsigned long key, DFBMatrix *pmatrix, unsigned long attrFlag  )
{
     int                      pos;
     ...
     while (element->value) {
          if ((element->value != DIRECT_HASH_ELEMENT_REMOVED && element->key == key )  && (!memcmp(pmatrix,&element->matrix, sizeof(element->matrix)))
            && (element->attrFlag == attrFlag))
               return pos;
                    ...
                    unsigned long attrFlag)
{
     ...
     element->key   = key;
     element->value = value;
     memcpy(&element->matrix, pmatrix, sizeof(element->matrix));
     element->attrFlag = attrFlag;
     hash->count++;
                    ...
                    unsigned long attrFlag )
{
     ...
     pos = locate_key( hash, key, pmatrix, attrFlag );
     if (pos == -1) {
