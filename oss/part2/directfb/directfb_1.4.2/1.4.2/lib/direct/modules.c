...
int
direct_modules_explore_directory_by_name( DirectModuleDir *directory ,
                                         const char      *soname )
{
#ifdef DIRECT_BUILD_DYNLOAD
	 DIR		   *dir;
	 ...
	 while (readdir_r( dir, &tmp, &entry ) == 0 && entry) {
		  ...
		  if(strcasecmp(soname, entry->d_name))
		  	continue;
		  ...
		  if ((handle = open_module( module )) != NULL) {
			   if (!module->loaded) {
					...
					if (func) {
						 ...
						 D_ERROR( "Direct/Modules: ... default contructor not found! error : %s \n",dlerror() );
					}
...
open_module( DirectModuleEntry *module )
{
     ...
     if (!handle)
          D_DLERROR( "Direct/Modules: Unable to dlopen `%s'! error:%s \n", buf ,dlerror());
