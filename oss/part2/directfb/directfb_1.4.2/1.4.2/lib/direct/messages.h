...
void DIRECT_API direct_messages_info         ( const char *func, const int line, const int pid, const char *format, ... )  D_FORMAT_PRINTF(4);

void DIRECT_API direct_messages_error        ( const char *func, const int line, const int pid, const char *format, ... )  D_FORMAT_PRINTF(4);
...
#define D_INFO(...)      do {                                                                  \
                              if (!(direct_config->quiet & DMT_INFO))                          \
                                   direct_messages_info( __FUNCTION__, __LINE__, getpid(), __VA_ARGS__);                        \
                         } while (0)

#define D_ERROR(...)     do {                                                                  \
                              if (!(direct_config->quiet & DMT_ERROR))                         \
                                   direct_messages_error( __FUNCTION__, __LINE__, getpid(), __VA_ARGS__ );                       \
                         } while (0)
