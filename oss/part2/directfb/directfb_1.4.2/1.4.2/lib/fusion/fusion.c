...
#define LINUX_MODE 3010
#define AOSP_MODE  1013
                 ...
                 FusionWorldShared **ret_shared )
{
     ...
     /* only master process needs to set permission and open flag O_CREAT */
     if (master) {
          fchmod( fd, fusion_config->secure_fusion ? 0640 : 0660 );
          ftruncate( fd, sizeof(FusionWorldShared) );
          if (DFB_SUPPORT_AN == 0){
              if (fchown(fd,0,LINUX_MODE) != 0) { // ROOT : MI
                  ret = errno2result(errno);
                  D_PERROR( "Fusion/SHM: Changing owner on %s failed\n", root_file );
                  goto out;
              }
          } else if (DFB_SUPPORT_AN == 1) { //for android two world
              if (fchown(fd,0,AOSP_MODE) != 0) { // ROOT : MEDIA
                  ret = errno2result(errno);
                  D_PERROR( "Fusion/SHM: Changing owner on %s failed\n", root_file );
                  goto out;
              }
              ...
              FusionWorld     **ret_world )
{
     ...
     if (role == FER_SLAVE && enter.fusion_id == FUSION_ID_MASTER) {
      ...
      unsigned long long tmp_shm_base;

     if (ioctl( fd, FUSION_SHM_GET_BASE, &tmp_shm_base )) {
          ret = errno2result(errno);
	...
	shm_base = (unsigned long)tmp_shm_base;
#else
     unsigned long shm_base;
     ...
     if (enter.fusion_id == FUSION_ID_MASTER) {
          ...
          ret = fusion_shm_pool_create( world, "Fusion Main Pool", fusion_config->mst_shm_main_pool_size*1024*1024,
                                        fusion_config->debugshm, &shared->main_pool );

          D_INFO("[DFB]dfb_config->mst_shm_main_pool_size :%d (MB)\n", fusion_config->mst_shm_main_pool_size);
          if (ret)
               goto error3;
     }
...
fusion_dispatch_loop( DirectThread *thread, void *arg )
{
     ...
     while (true) {
          ...
          if (len < 0) {
			   ...
			   if (errno == EMSGSIZE)
			   {
			   		printf("[kmjdbg]: errno:%d PID:%d tid:0x%x\n", errno,getpid(),pthread_self());
					break;
			   }
     ...
     printf( "[kmjdbg]Fusion/Receiver: reading from fusion device failed.......!!! PID:%d tid:0x%x\n",getpid(),pthread_self());
              ...
              FusionWorld     **ret_world )
{
     ...
     if (world->fusion_id == FUSION_ID_MASTER) {
          ...
          /* Create the main pool. */
          ret = fusion_shm_pool_create( world, "Fusion Main Pool", 0x200000,
                                        fusion_config->debugshm, &shared->main_pool );
          if (ret)
               goto error3;
     }
