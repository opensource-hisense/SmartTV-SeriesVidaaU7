...
D_DEBUG_DOMAIN( Voodoo_Manager_Socket, "Voodoo/Manager/Socket", "Voodoo Manager Socket" );


/**********************************************************************************************************************/

#define IN_BUF_MAX   (32 * 1024)
#define OUT_BUF_MAX  (32 * 1024)
...
static void
hexdump( u8 *bytes, size_t length )
{
     size_t i;

     for (i=0; i<length; i+=16) {
          D_DEBUG_AT( Voodoo_Manager_Socket, "%08d: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
                      i, bytes[i+0], bytes[i+1], bytes[i+2], bytes[i+3], bytes[i+4], bytes[i+5], bytes[i+6], bytes[i+7], bytes[i+8], bytes[i+9], bytes[i+10], bytes[i+11], bytes[i+12], bytes[i+13], bytes[i+14], bytes[i+15] );
     }
}
...
static void *manager_socket_loop  ( DirectThread *thread, void *arg );
                       ...
                       VoodooManager **ret_manager )
{
     ...
     if (1) {
          ...
          if (pipe( fds )) {
               D_PERROR( "Voodoo/Manager: Could not create pipe!\n" );
               // FIXME: cleanup
               return DR_IO;
          }

          manager->fd_sleep  = fds[0];
          manager->fd_wakeup = fds[1];
     }
     ...
     if (1) {
          manager->output.thread = direct_thread_create( DTT_OUTPUT, manager_socket_loop,
                                                         manager, "Voodoo Socket" );
...
voodoo_manager_quit( VoodooManager *manager )
{
     ...
     write( manager->fd_wakeup, "", 1 );

     direct_thread_join( manager->output.thread );

     if (1) {
          close( manager->fd_sleep );
          close( manager->fd_wakeup );
     }
...
static void *
manager_socket_loop( DirectThread *thread, void *arg )
{
     int            len;
     struct pollfd  pf[2];
     VoodooManager *manager = arg;
     ...
     while (!manager->quit) {
          ...
          D_DEBUG_AT( Voodoo_Manager_Socket, "%s( %p )\n", __FUNCTION__, manager );

          pf[0].fd      = manager->fd;
          pf[0].events  = 0;
          pf[0].revents = 0;

          pf[1].fd      = manager->fd_sleep;
          pf[1].events  = POLLIN;
          pf[1].revents = 0;
          ...
          if (manager->input.end != manager->input.max) {
               ...
               D_DEBUG_AT( Voodoo_Manager_Socket, "  -> may receive, adding POLLIN...\n" );
          }

          if (manager->output.start != manager->output.end) {
               pf[0].events |= POLLOUT;

               D_DEBUG_AT( Voodoo_Manager_Socket, "  -> data to send, adding POLLOUT...\n" );
          }

          switch (poll( pf, 2, 1000 )) {
               case -1:
                    if (errno != EINTR) {
                         D_PERROR( "Voodoo/Output: Could not poll() the socket!\n" );
                         usleep( 200000 );
               ...
               case 1:
                    if (pf[1].revents & POLLIN) {
                         ...
                         D_DEBUG_AT( Voodoo_Manager_Socket, "  -> woke up!\n" );

                         read( manager->fd_sleep, buf, 512 );
                    }
                    break;

               case 0:
                    D_DEBUG_AT( Voodoo_Manager_Socket, "  -> timeout\n" );
                    continue;
          }

          D_DEBUG_AT( Voodoo_Manager_Socket, "%s( %p )\n", __FUNCTION__, manager );

//          pf[0].revents = POLLIN|POLLOUT;

          if (pf[0].revents & POLLIN) {
               D_DEBUG_AT( Voodoo_Manager_Socket, "  -> POLLIN\n" );
               ...
               if (manager->input.end != manager->input.max) {
                    if (!manager->quit) {
                         ...
                         if (len < 0) {
                              ...
                              D_DEBUG_AT( Voodoo_Manager_Socket, "Voodoo/Input: Received %d bytes...\n", len );
     
                              hexdump( manager->input.buffer + manager->input.end, len );
          ...
          if (pf[0].revents & POLLOUT) {
               D_DEBUG_AT( Voodoo_Manager_Socket, "  -> POLLOUT\n" );
               ...
               if (manager->output.start != manager->output.end) {
                    if (!manager->quit) {
                         ...
                         size = manager->output.end - manager->output.start;
                         if (size > 1408)
                              size = 1408;
