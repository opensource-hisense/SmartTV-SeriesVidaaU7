                   ...
                   DFBWindowEvent *event )
{
     ...
     event->device_id = sawman->manager.event.ex_device_id;
                      ...
                      SaWManUpdateFlags    update_flags )
{
     ...
     if (!SAWMAN_VISIBLE_WINDOW(window) && !(update_flags & SWMUF_FORCE_INVISIBLE)) {
          D_DEBUG_AT( SaWMan_Update, "  -> window %d was not visible window!\n", window->id );
          return DFB_OK;
     }
                                ...
                                DFBWindowCursorFlags   flags )
{
     ...
     if (window->config.cursor_flags != flags) {
          ...
          if (sawwin == sawman->focused_window) {
			   ...
			   stack->cursor.flags |= update;

               if (update && stack->cursor.enabled)
                          ...
                          int              y )
{
     ...
     fusion_vector_foreach_reverse (sawwin, i, sawman->layout) {
          ...
          tx = (s64) x * (s64) tier->size.w / (s64) tier_0->size.w;
          ty = (s64) y * (s64) tier->size.h / (s64) tier_0->size.h;
                                   ...
                                   int             *ret_cy )
{
     ...
     sawman_tier_by_layer( sawman, DLID_PRIMARY, &tier_0 );
     D_MAGIC_ASSERT( tier_0, SaWManTier );	 
...
sawman_update_visible( SaWMan *sawman )
{
     ...
     update_visible( sawman, sawman->layout.count - 1, 0, 0, sawman_config->max_resolution.w, sawman_config->max_resolution.h, false );
     update_visible( sawman, sawman->layout.count - 1, 0, 0, sawman_config->max_resolution.w, sawman_config->max_resolution.h, true );
     ...
     for (i=0; i<sawman->layout.count; i++) {
          ...
          // direct_log_printf(NULL,"%s[pid = %d][0x%x,%d] \n",__FUNCTION__,getpid(),window->id,window->left.visible.num_regions);
