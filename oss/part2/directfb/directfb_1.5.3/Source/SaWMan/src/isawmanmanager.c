                                 ...
                                 SaWManWindowConfig       *config )
{
     ...
     */
     ...
     ret = SaWManManager_SetWindowConfig( data->manager, sawwin, config, flags );

     DIRECT_INTERFACE_DBG_DELTA_END(DDDT_ISAWMANMANAGER_SETWINDOWCONFIG);

     return ret;
}

static DirectResult
ISaWManManager_LockContext( ISaWManManager       *thiz,
                                SaWManWindowHandle    handle)
{
    SaWManWindow   *window  = (SaWManWindow*)handle;
    if (!window)
    {
      direct_log_printf(NULL,"window is null pointer %s [pid=%d]\n", __FUNCTION__, getpid());
      return DFB_INVARG;
    }

    dfb_layer_context_lock(window->stack->context);
...
static DirectResult
ISaWManManager_UnLockContext( ISaWManManager       *thiz,
                                SaWManWindowHandle    handle)
{
    SaWManWindow   *window  = (SaWManWindow*)handle;
    if (!window)
    ...
    dfb_layer_context_unlock(window->stack->context);
...
ISaWManManager_Lock( ISaWManManager *thiz )
{
     ...
     DIRECT_INTERFACE_DBG_DELTA_END(DDDT_ISAWMANMANAGER_LOCK);
                          ...
                          SaWManManager  *manager )
{
     ...
     thiz->LockContext     = ISaWManManager_LockContext;
     thiz->UnLockContext   = ISaWManManager_UnLockContext;
