...
DirectResult
sawman_sync_buffer(SaWMan              *sawman,
                        SaWManTier           *tier)
{
                        ...
                        DFBSurfaceFlipFlags  flags )
{
     ...
     direct_list_foreach (tier, sawman->tiers) {
          ...
          if (single && !border_only) {
               ...
               /* Not do single window optimized ? */
               if (!sawman_config->single_optimized[tier->layer_id])
                    goto no_single;
...
// for android project memory shrink. 
#ifdef CC_SHRINK_DFB_BUFFER_CN
...
no_single:
          ...
          if (!tier->active) {
               D_DEBUG_AT( SaWMan_Auto, "  -> Activating tier... [%dx%d]\n", tier->size.w, tier->size.h );
