...
static const char *config_usage =
     ...
     "  no-single_optimized=<layer-id>     Set Layer do not single window optimized\n"
     "\n";
...
config_allocate()
{
     ...
     sawman_config->cursor.layer_id = MT53_LAYER_CURSOR;
#endif

     sawman_config->m_ContainerLayerId = MT53_LAYER_VDP1;

     sawman_config->max_resolution.w = 1920;
	 sawman_config->max_resolution.h = 1080;

     for(i=0;i<4;i++) {
          sawman_config->single_optimized[i] = true;
     }

	 dfb_set_cursor_border(0,0,sawman_config->max_resolution.w,0,sawman_config->max_resolution.h);
...
sawman_config_set( const char *name, const char *value )
{
     ...
     if (strcmp(name, "no-single_optimized") == 0 ) {
         if (value) {
             int tier;

             if(sscanf( value, "%d", &tier) < 1) {
                 D_ERROR("SaWMan/Config '%s': Could not parse dimension!\n", name);
                 return DFB_INVARG;
             }

             sawman_config->single_optimized[tier] = false;
         }
         else {
              D_ERROR("SaWMan/Config '%s': No width and height specified!\n", name);
              return DFB_INVARG;
...
sawman_config_init( int *argc, char **argv[] )
{
     ...
     if (!sawman_config) {
          ...
          /* Read system settings for adaptable */
          ret = sawman_config_read( LINUX_3RD_RW_PATH"/sawmanrc" );
          if (ret  &&  ret != DFB_IO)
               return ret;
