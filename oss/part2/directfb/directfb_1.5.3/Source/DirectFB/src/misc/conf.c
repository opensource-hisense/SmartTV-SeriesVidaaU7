...
static const char *config_usage =
     ...
     "\n"
     " Control osd work mode:\n"
     "  (no-)osd-iommu\n"
     " Control use OpenGL ES to compose.(Notice: this option just do compose)\n"
     " (dis)enable-gles\n"
     "\n";
...
static void config_allocate( void )
{
...
#else
     dfb_config->surface_shmpool_size     = 180 * 1024 * 1024;
#endif
     ...
     dfb_config->dump_stack_fg      = 0;
     for (i =0; i < DDDLT_MAX; i ++) {
        dfb_config->delay_time[i] = NULL;
     }

     dfb_config->osd_iommu = false;
     dfb_config->enable_gles = true;    /* enable by default */
     dfb_config->debug_performance = 0;
}
...
bool dfb_config_get_dump_stack(DFB_DBG_DUMP_STACK domain)
{
    if (dfb_config->dump_stack_fg & domain)
        return true;
    else
        return false;
}

u32 dfb_config_get_delay_time(DFB_DBG_DELAY_TIME domain)
{
    if (dfb_config->delay_time[domain])
        return *dfb_config->delay_time[domain];
    else
        return 0;
}

static DFBResult
dump_stack_main(const char* domain)
{
    if (!dfb_config)
    {
        return DFB_FAILURE;
    ...
    if ( strcmp(domain, "layer_flip") == 0 ) {
        dfb_config->dump_stack_fg |= DDDS_LAYER_FLIP;
    }
    else if ( strcmp(domain, "layer_set") == 0 ) {
        dfb_config->dump_stack_fg |= DDDS_LAYER_SET;
    }
    else if ( strcmp(domain, "layer_set_order") == 0) {
        dfb_config->dump_stack_fg |= DDDS_LAYER_SER_ORDER;
    }
    else if ( strcmp(domain, "layer_enable") == 0 ) {
        dfb_config->dump_stack_fg |= DDDS_LAYER_ENABLE;
    }
    else if ( strcmp(domain, "surface_alloc") == 0) {
        dfb_config->dump_stack_fg |= DDDS_SURFACE_ALLOC;
    }
    else if ( strcmp(domain, "window_create") == 0 ) {
        dfb_config->dump_stack_fg |= DDDS_WINDOW_CREATE;
    }
    else
    {
        D_WARN("Unsupport dump stack domain\n");
    }
    return DFB_OK;
}

static DFBResult
dfb_config_set_delay(const char* name, const char* value)
{
    if (!dfb_config)
    {
        return DFB_FAILURE;
    ...
    if (strcmp(name, "dfbdelay:flip") == 0) {
        if (!dfb_config->delay_time[DDDLT_LAYER_FLIP]) {
            dfb_config->delay_time[DDDLT_LAYER_FLIP] = D_CALLOC(DDDLT_MAX ,sizeof(u32));
        }
        if (value)
        {
            *dfb_config->delay_time[DDDLT_LAYER_FLIP] = direct_strtoul(value, NULL, 10);
        } else {
            *dfb_config->delay_time[DDDLT_LAYER_FLIP] = 1000;
        }
    }
}
...
#endif
     if (strcmp (name, "debug-performance") == 0) {
        if (value)
        {
            unsigned int flags;

            if (direct_sscanf( value, "%x", &flags ) < 1) {
                D_ERROR( "DirectFB/Config 'agp': "
                            "invalid agp flags '%s'!\n", value );
                return DFB_INVARG;
            }

            dfb_config->debug_performance = flags;
        }
        else
            dfb_config->debug_performance = 0xFFFFFFFF;
     } else
     if (strcmp (name, "disable-gles") == 0) {
          dfb_config->enable_gles = false;
     } else
     if (strcmp (name, "enable-gles") == 0) {
           dfb_config->enable_gles = true;
     } else
     if (strcmp (name, "no-osd-iommu" ) == 0) {
          dfb_config->osd_iommu = false;
     } else
     if (strcmp (name, "osd-iommu" ) == 0) {
          dfb_config->osd_iommu = true;
     } else
     if (strcmp (name, "dumpstack") == 0) {
          dump_stack_main(value);
     } else
     if (strncmp (name, "dfbdelay:", 9) == 0) {
          dfb_config_set_delay(name, value);
     } else
...
DFBResult dfb_config_init( int *argc, char *(*argv[]) )
{
     ...
     ret = dfb_config_read( LINUX_ROOTFS_PATH"/etc/directfbrc" );
     if (ret  &&  ret != DFB_IO)
          return ret;
     ...
     dfb_config_read(LINUX_3RD_RW_PATH"/directfbrc");
     ...
     if (prog && prog[0]) {
          int  len = strlen( LINUX_ROOTFS_PATH"/etc/directfbrc." ) + strlen(prog) + 1;
          char buf[len];

          direct_snprintf( buf, len, LINUX_ROOTFS_PATH"/etc/directfbrc.%s", prog );
