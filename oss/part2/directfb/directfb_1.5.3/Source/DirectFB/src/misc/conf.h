...
#define DDEBUG_PM_BLIT_MASK              (1<<0)
#define DDEBUG_PM_DRAW_MASK              (1<<1)
...
typedef struct
{
    ...
    u32           dump_stack_fg;                 /* Dump interface call stack */ 

    u32*          delay_time[DDDLT_MAX];

    bool          osd_iommu;

    bool          enable_gles;                   /* OpenGL ES support for DFB */

    u32           debug_performance;
} DFBConfig;
