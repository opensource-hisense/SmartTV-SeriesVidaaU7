                        ...
                        DFBSurfaceLock2Parameter *operation )
{
     ...
     if (!operation)
          return DFB_INVARG;
     else if(operation->size != sizeof(DFBSurfaceLock2Parameter)) {
         printf("[DFB] %s(%d), different DFBSurfaceLock2Parameter size %d vs %d (pid = %d)\n",
                 __FUNCTION__, __LINE__, operation->size, sizeof(DFBSurfaceLock2Parameter), getpid());
         return DFB_FAILURE;
     }
     ...
     ret = dfb_surface_lock_buffer( data->surface, role, CSAID_DMA, access, &data->lock );
     if (ret)
          return ret;
     ...
     u8_phy  = data->lock.phys;

     operation->ret_ptr = (void *)u4_addr + data->lock.pitch * data->area.current.y +
                      DFB_BYTES_PER_LINE( data->surface->config.format, data->area.current.x );
     ...
     if (data->surface->config.caps & DSCAPS_VIDEOONLY)
     {
         if (data->lock.dma_fd <= 0)
         {
            D_ERROR("[DFB] %s(%d), dma_buf fd: %d\n", __FUNCTION__, __LINE__, data->lock.dma_fd);
            return DFB_FAILURE;
         }
         operation->ret_type = DSLT_PHYS;
         operation->ret_phys = u8_phy;
         operation->ret_fd   = data->lock.dma_fd;
     }
     else if (data->surface->config.caps & DSCAPS_IOMMU)
     {
        if (data->lock.iova <=0 || data->lock.dma_fd <= 0)
        {
            D_ERROR("[DFB] %s(%d), Iova address: 0x%x, dma_buf fd: %d\n", __FUNCTION__, __LINE__, data->lock.iova, data->lock.dma_fd);
            return DFB_FAILURE;
        }
        operation->ret_type = DSLT_IOVA;
        operation->ret_iova = data->lock.iova;
        operation->ret_fd   = data->lock.dma_fd;
     }
     else
     {
                                      ...
                                      CoreDFB                *core )
{
     ...
     thiz->Lock2 = IDirectFBSurface_Lock2;
