               ...
               CoreSurface            **ret_surface )
{
     ...
     config.buffermode    = DLBM_WINDOWS;       /* Buffermode fix to WINDOWS */
                   ...
                   CoreWindow                 **ret_window )
{
     ...
     colorspace   = desc->colorspace;
               ...
               surface_caps = desc->surface_caps & (DSCAPS_INTERLACED    | DSCAPS_SEPARATED  |
                                                    ...
                                                    DSCAPS_VIDEOONLY | DSCAPS_FROM_3DMM|DSCAPS_FOUCE_VIDEOONLY|DSCAPS_TRIPLE);
#else
          surface_caps = desc->surface_caps & (DSCAPS_INTERLACED    | DSCAPS_SEPARATED  |
                                               DSCAPS_PREMULTIPLIED | DSCAPS_DEPTH      |
                                               DSCAPS_STATIC_ALLOC  | DSCAPS_SYSTEMONLY |
     ...
     /* Dump window create call stack ? */
     if (dfb_config_get_dump_stack(DDDS_WINDOW_CREATE)) {
         D_INFO( "create-window   %4dx%4d %6s, caps 0x%08x, surface-caps 0x%08x, ID %u",
                 window->config.bounds.w, window->config.bounds.h, dfb_pixelformat_name(pixelformat),
                 window->caps, surface_caps, window->id );
         dfb_print_trace();
     }
