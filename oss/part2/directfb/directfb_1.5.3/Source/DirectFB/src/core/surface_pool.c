                             ...
                             unsigned int            *ret_num )
{
     ...
     D_DEBUG_AT(Core_SurfacePool, "%s[0x%x,0x%x,0x%x,0x%x,0x%x] \n",__FUNCTION__,
        surface->type,
        buffer->policy,
     ...
     if( DFB_SYSTEMONLY && !DSCAPS_FROM_VIDEO(surface->config.caps) && (!(SURFACE_TYPE_VIDEO&type) || dfb_config->osd_iommu))
     {
                            ...
                            CoreSurfaceAllocation  **ret_allocation )
{
    ...
    if (ret && ret != DFB_NOVIDEOMEMORY) 
    {
     ...
     if (!allocation) {
          ...
          direct_log_printf(NULL,"%s DFB_FAILURE[%d] \n",__FUNCTION__,ret);
          return DFB_FAILURE;
     }
                       ...
                       CoreSurfaceBufferLock *lock )
{
     ...
     if(funcs == NULL)
     {
        D_ERROR( "Core/SurfacePool: function is NULL, fail!\n" );
        return DFB_FAILURE;
     }
