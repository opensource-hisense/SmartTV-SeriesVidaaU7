...
CoreSurfaceAccessorID   accessor    = CSAID_NONE;
    ...
    buffer      = allocation->buffer;
     ...
     D_DEBUG_AT( Core_SurfBuffer, "%s[pid= %d,%d][0x%x,0x%x][0x%x,0x%x,0x%x] [0x%x,0x%x]\n",
                __FUNCTION__,
                getpid(),
                dfb_gfxcard_get_num(),
    ...
    if(accessor == CSAID_CPU)
    {
        ...
        {
             //dfb_gfxcard_sync();
             dfb_gfxcard_flush_read_cache(lock);
             if (!buffer->locked) {
        ...
        if (access & CSAF_WRITE) {
             ...
             if (allocation->accessed[CSAID_GPU] & (CSAF_READ)) {
                 /** ...wait for the operateion to finish. */
                 //dfb_gfxcard_sync();
    ...
    if(accessor == CSAID_GPU && access & CSAF_READ)
    {
        ...
        {
            /** ...flush texture cache */
            dfb_gfxcard_flush_texture_cache(lock);
                         ...
                         CoreSurfaceBufferLock  *lock )
{
     ...
     switch (accessor) {
          ...
          case CSAID_GPU:
               D_DEBUG_AT( Core_SurfBuffer, "  -> GPU/GFX %s%s\n",
                           (access & CSAF_READ) ? "READ" : "", (access & CSAF_WRITE) ? "WRITE" : "" );
               break;

         case CSAID_DMA:
               D_DEBUG_AT( Core_SurfBuffer, "  -> DMA %s%s\n",
                           (access & CSAF_READ) ? "READ" : "", (access & CSAF_WRITE) ? "WRITE" : "");
               break;

         case CSAID_OSD:
               D_DEBUG_AT( Core_SurfBuffer, "  -> OSD %s%s\n",
                           (access & CSAF_READ) ? "READ" : "", (access & CSAF_WRITE) ? "WRITE" : "");
               break;

         case CSAID_IMGRZ:
               D_DEBUG_AT( Core_SurfBuffer, "  -> IMGRZ %s%s\n",
                           (access & CSAF_READ) ? "READ" : "", (access & CSAF_WRITE) ? "WRITE" : "");
               break;
