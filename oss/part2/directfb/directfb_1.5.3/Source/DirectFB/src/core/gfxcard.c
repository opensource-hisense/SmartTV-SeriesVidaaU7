...
#define __DEBUG_TIMESTAMP(name)             \
    if (dfb_config->debug_performance & DDEBUG_PM_BLIT_MASK) \
    { \
        static struct timeval __gfxcard_##name; \
        static unsigned long long __gfxcard_##name##_backup; \
        unsigned long long __gfxcard_##name##_value = 0; \
        gettimeofday(&__gfxcard_##name, 0); \
        __gfxcard_##name##_value = __gfxcard_##name.tv_sec * 1000000 + __gfxcard_##name.tv_usec; \
        D_INFO("timestamp to call %s is %lld ms, interval of last is %lld ms\n", #name, __gfxcard_##name##_value/1000, (__gfxcard_##name##_value - __gfxcard_##name##_backup)/1000); \
        __gfxcard_##name##_backup = __gfxcard_##name##_value; \
    }

static bool __check_GPU_compose(CardState *state, DFBRectangle* drect)
{
    if ( DFB_COLOR_IS_RGB(state->source->config.format) &&
         DFB_COLOR_IS_RGB(state->destination->config.format) &&
         (drect->x == 0 && drect->y ==0) )
         return true;
    else
         return false;
                              ...
                              DFBGraphicsCoreShared *shared )
{
     ...
     if (data->driver_funcs) {
          ...
          card->device_data   =
          shared->device_data = D_CALLOC( 1, shared->driver_info.device_data_size );

          ret = funcs->InitDriver( card, &card->funcs,
                                   card->driver_data, card->device_data, core );
          if (ret) {
               D_FREE(shared->device_data );
                        ...
                        DFBGraphicsCoreShared *shared )
{
     ...
     if (data->driver_funcs) {
          ...
          data->device_data = shared->device_data = D_CALLOC( 1, shared->driver_info.device_data_size );;

          ret = funcs->InitDriver( card, &card->funcs,
                                   card->driver_data, card->device_data, core );
          if (ret) {
               D_FREE( data->driver_data );
               D_FREE(data->device_data);
               data = NULL;
               return ret;
          }
     }
...
dfb_gfxcard_state_check( CardState *state, DFBAccelerationMask accel )
{
     ...
     if(((state->blittingflags & DSBLIT_BLEND_ALPHACHANNEL)||(accel&DFXL_STRETCHBLIT))&&
         ((state->blittingflags & DSBLIT_ROTATE90)||
         (state->blittingflags & DSBLIT_ROTATE180)||
         (state->blittingflags & DSBLIT_ROTATE270)))
     {
         return false;
     }
...
void dfb_gfxcard_blit( DFBRectangle *rect, int dx, int dy, CardState *state )
{
         ...
         dfb_gfxcard_state_acquire( state, DFXL_BLIT ))
     {
          ...
          __DEBUG_TIMESTAMP(before_Blit);

          if (card->funcs.BlitGLES2 && __check_GPU_compose(state, &drect))
          {
                 hw = card->funcs.BlitGLES2( card->driver_data, card->device_data, state, rect, &drect );
                 if (hw)
                    card->funcs.FinishGLES2( card->driver_data, card->device_data );
          }
          ...
          __DEBUG_TIMESTAMP(after_Blit);

          dfb_gfxcard_state_release( state );

          __DEBUG_TIMESTAMP(release_state);
     }
                              ...
                              CardState *state )
{
         ...
         dfb_gfxcard_state_acquire( state, DFXL_STRETCHBLIT ))
     {
          ...
          __DEBUG_TIMESTAMP(before_StretchsBlit);
          if (card->funcs.BlitGLES2 && __check_GPU_compose(state, drect))
          {
          ...
          __DEBUG_TIMESTAMP(after_StretchsBlit);
                                    ...
                                    CardState *state )
{
     ...
     dfb_state_unlock( state );
     DIRECT_INTERFACE_DBG_DELTA_END(DDDT_CARD_TEXTURE_TRIANGLES);
}
...
void dfb_gfxcard_flush_texture_cache(CoreSurfaceBufferLock *lock)
{
     D_ASSUME( card != NULL );
     ...
     if (card && card->funcs.FlushTextureCache && lock)
          card->funcs.FlushTextureCache( card->driver_data, card->device_data, lock );
}
...
void dfb_gfxcard_flush_read_cache( CoreSurfaceBufferLock *lock)
{
     D_ASSUME( card != NULL );
     ...
     if (card && card->funcs.FlushReadCache && lock)
          card->funcs.FlushReadCache( card->driver_data, card->device_data, lock );
}
