...
dfb_layer_context_activate( CoreLayerContext *context )
{
     ...
     fusion_vector_foreach (region, index, context->regions) {
          ...
          {
               D_WARN( "%s[%d][0x%x] ******* >> could not activate region!", __func__, __LINE__, region->state );
          }
     }
                                     ...
                                     const DFBDisplayLayerConfig *config )
{
     ...
     if (dfb_config_get_dump_stack(DDDS_LAYER_SET)) {
         D_INFO( "%s, Layer%d, Flags: 0x%x, BufferMode: 0x%x, Width: %d, Height: %d, Options: 0x%x\n", __func__, context->layer_id, config->flags, config->buffermode, config->width, config->height, config->options );
         dfb_print_trace();
     }
     ...
     if (context->stack) {
         ...
         /* Force update context stack when Layer-mode changed */
                  ...
                  CoreLayerRegionConfig *config )
{
     ...
     if (funcs->AllocateSurface) {
          ...
          D_DEBUG_AT( Core_LayerContext,"%s[caps: 0x%x, surface: 0x%x] [type: 0x%x] \n",__FUNCTION__, caps,config->surface_caps, type );
...
deallocate_surface( CoreLayer *layer, CoreLayerRegion *region )
{
     ...
     if (surface) {
          ...
          /* Destroy the buffers in surface */
          dfb_surface_destroy_buffers( surface );
