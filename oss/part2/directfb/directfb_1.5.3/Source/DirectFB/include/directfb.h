...
typedef enum {
     ...
     DSCAPS_ALL          = 0xFF1333F7,  /* All of these. */
...
typedef enum {
     DDDS_LAYER_FLIP        = 0x00000001,       /* dump layer flip stack */
     DDDS_LAYER_SET         = 0x00000002,       /* dump layer set stack */
     DDDS_LAYER_SER_ORDER   = 0x00000004,       /* dump layer set order */
     DDDS_LAYER_ENABLE      = 0x00000008,       /* dump layer enable */
     DDDS_SURFACE_ALLOC     = 0x00000010,       /* dump surface allocate */
     DDDS_WINDOW_CREATE     = 0x00000020,       /* dump window create */
}DFB_DBG_DUMP_STACK;
...
typedef enum {
     ...
     DWCAPS_STEREO       = 0x00020000,  /* Window has 2 independent sets of surface buffers (left eye & right eye 
                                           buffers), each with unique content. The number of buffers in each set is
     ...
     DWCAPS_ALL          = 0x0002113F   /* All of these. */
...
typedef enum {
     ...
     DSLT_IOVA                = 0x00000001,       /* union return ret_iova */
     DSLT_PHYS                = 0x00000002        /* union return ret_phys */
} DFBSurfaceLock2RetType;

typedef struct {
    size_t                              size;        /* Input sizeof this structure from caller */

    void                               *ret_ptr;     /* Virtual address */
    union {
        u32                             ret_iova;    /* return IOVA address */
        u64                             ret_phys;    /* return Physical address */
    };
    int                                 ret_pitch;   /* Line pitch */
    int                                 ret_fd;      /* dma_buf fd */
    DFBSurfaceLock2RetType              ret_type;    /* Judge whether it's IOVA or Physical */
} DFBSurfaceLock2Parameter;
...
D_DEFINE_INTERFACE(   IDirectFBSurface,
   ...
   /** Blitting control **/
     ...
     DFBResult (*Lock2) (
         IDirectFBSurface              *thiz,
         DFBSurfaceLockFlags            flags,
         DFBSurfaceLock2Parameter      *operation
     );
)
...
/*#ifdef GFX_IOMMU_IOVA_SUPPORT need to be kept. remove for seperate control*/
    unsigned int devIdx; /* 0: gfx, 1:gcpu, 2: imgrz 5:osd*/
    unsigned int pgd;
    unsigned int iova;
...
/*
 * IOVA device for MTK HW
 */
typedef enum {
    E_IDEV_GFX     = 0x0,   /* default is GFX iova */
    E_IDEV_OSD     = 0x1,
    E_IDEV_IMGRZ   = 0x2,
    E_IDEV_MAX     = 0x3
} DFB_IOVA_DEV_T;
...
typedef struct {
     ...
     int   fd;          /* file descripter for dma_buf using */
     u32   iova[E_IDEV_MAX];
...
#define DFB_SYSTEMONLY                              (mt53_support_mmu())
...
extern int system_mmu_pin_mem_osd( DFB_IOMMU_PIN_RANGE_T *pt_page );
extern int system_mmu_unpin_mem_osd( DFB_IOMMU_PIN_RANGE_T *pt_page );
extern int system_mmu_get_dmafd( DFB_IOMMU_PIN_RANGE_T *pt_page );
extern u32 system_mmu_transform_gfx2osd_iova( u32 u4_src, size_t u4_size);
extern int system_mmu_free_osd_iova(u32 u4_src, size_t u4_size);
...
extern bool dfb_config_get_dump_stack(DFB_DBG_DUMP_STACK domain);
extern u32 dfb_config_get_delay_time(DFB_DBG_DELAY_TIME domain);
