              ...
              FusionWorld     **ret_world )
{
	...
	shm_base = (unsigned long)tmp_shm_base;
     ...
     D_DEBUG_AT( Fusion_Main, "     -> share base: 0x%x\n", shm_base );
     ...
     if (enter.fusion_id == FUSION_ID_MASTER) {
          ...
          ret = fusion_shm_pool_create( world, "Fusion Main Pool", 0x800000,
							            fusion_config->debugshm, &shared->main_pool );
#else
          ret = fusion_shm_pool_create( world, "Fusion Main Pool", 0x300000,
                                        fusion_config->debugshm, &shared->main_pool );
#endif
          if (ret)
               goto error3;
              ...
              FusionWorld     **ret_world )
{
     ...
     if (world_index < 0) {
          ...
          for (world_index=0; world_index<FUSION_MAX_WORLDS; world_index++) {
               ...
               len = snprintf( addr.sun_path, sizeof(addr.sun_path), LINUX_TMP_PATH"/.fusion-%d/", world_index );
     ...
     if (id == FUSION_ID_MASTER) {
          ...
          snprintf( addr.sun_path, sizeof(addr.sun_path),
                    LINUX_TMP_PATH"/.fusion-%d/%lx", world_index, FUSION_ID_MASTER );
             ...
             bool         emergency )
{
     ...
     if (clear) {
          ...
          len = snprintf( buf, sizeof(buf), LINUX_TMP_PATH"/.fusion-%d/", world_index );
          dir = opendir( buf );
          if (dir) {
