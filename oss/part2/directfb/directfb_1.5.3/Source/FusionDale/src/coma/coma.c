               ...
               void         **ret_ptr )
{
    ...
    ret = fusion_skirmish_prevail( &shared->lock );
    if (ret) {
      //direct_log_printf( NULL, "[chenli]Coma/Core: Could not lock core to remove component!\n" );
      return DR_FAILURE;
    }
    ptr = SHCALLOC( coma->shared->shmpool, 1, bytes );
    //direct_log_printf( NULL,"[chenli]coma_allocate coma:%p, size:%d, pt:%p\n", coma, bytes, ptr);
    if (!ptr)
    {
      fusion_skirmish_dismiss( &shared->lock );
      return D_OOSHM();
                 ...
                 void *ptr )
{
     ...
     if (ret) {
		...
		//direct_log_printf( NULL,"[chenli]coma_deallocate coma:%p, pt:%p\n", coma, ptr);
