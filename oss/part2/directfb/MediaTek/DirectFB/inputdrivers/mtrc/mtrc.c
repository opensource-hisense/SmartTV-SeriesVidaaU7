...
#define _syscall0(type, name)                                                                                                                        \
    ...
    {                                                                                                                                                \
        ...
        long __res;                                                                                                                                  \
        __asm__ __volatile__(__syscall(name) : "=r"(__res_r0) : __SYS_REG_LIST() : "memory");                                                        \
        __res = __res_r0;                                                                                                                            \
...
#define printf_display(x, format, arg...)                                                                                                            \
    {                                                                                                                                                \
        if (x)                                                                                                                                       \
            printf(format, arg);                                                                                                                     \
    }
...
void mtal_ioctl_prepare(const char* funcname) { UNUSED(funcname); }
static void send_symbol(RCInputData* data, DFBInputDeviceKeySymbol symbol, int key_code, bool down)
{
    DFBInputEvent event;
    ...
    printf("BTN_%s: 0x%x policy: %d prio %d\n", (down ? "DOWN" : "UP"), event.key_code, sched_getscheduler(gettid()), (int)rParam.sched_priority);

    dfb_input_dispatch(data->device, &event);
}

static u32 translate_KeySymbol(u32 code)
{
...
*/
static void* input_thread(DirectThread* thread, void* arg)
{
    ...
    int i4Ret;
    MTIR_TYPE_T ir_type = MTIR_TYPE_DEFAULT;
#ifdef FACTORYMODE
    int fd;
...
#ifndef __USERDRV__
    i4Ret = mtal_ioctl(_i4DeviceHandle, MTAL_IO_MTPERIPHERAL_IR_INIT, &ir_type);
#else
    i4Ret = mtal_ipc(MTAL_IO_MTPERIPHERAL_IR_INIT, (VOID*)&ir_type, sizeof(MTIR_TYPE_T));
#endif
    if (i4Ret) {
    ...
    while (!data->quit_thread) {
        ...
        MTAL_IOCTL_3UNARG_T rArg;
        rArg.aUNArg[0].value = &u4Key;
        rArg.aUNArg[1].value = &u4BtnState;
        rArg.aUNArg[2].value = &rRaw;
#ifndef __USERDRV__
        i4Ret = mtal_ioctl(_i4DeviceHandle, MTAL_IOUN_MTPERIPHERAL_IR_WAIT, &rArg);
#else
        i4Ret = mtal_ipc(MTAL_IOUN_MTPERIPHERAL_IR_WAIT, (VOID*)&rArg, sizeof(MTAL_IOCTL_3UNARG_T));
#endif
        ...
        if (u4Key == BTN_NONE) {
            D_DEBUG_AT(RCInput, "[DirectFB]: BTN_NONE..discard\n");
            usleep(10000);
            continue; // return;
        }
        ...
        if (u4Key == BTN_IR_MOUSE_MOVE) {
            if (u4BtnState) {
                if (((u32)rRaw.au1Data[1])) // au1Data[1] should be placed x offset at IRRX layer
                {
                    ...
                    D_DEBUG_AT(RCInput, "[mtrc]IR_MOUSE_Info : pos mouse dx: 0x%02x\n", ((int)((INT8)(rRaw.au1Data[1]))));
                    dfb_input_dispatch(data->device, &event);
                }
                if (((u32)rRaw.au1Data[2])) // au1Data[2] should be placed y offset at IRRX layer
                {
                    ...
                    D_DEBUG_AT(RCInput, "[mtrc]IR_MOUSE_Info : pos mouse dy: 0x%02x\n", ((int)((INT8)(rRaw.au1Data[2]))));
                    dfb_input_dispatch(data->device, &event);
                }
                if (((u32)rRaw.au1Data[3])) // au1Data[3] should be placed z offset at IRRX layer
                {
                    ...
                    D_DEBUG_AT(RCInput, "IR_MOUSE_Info:pos mouse dz: 0x%02x\n", ((int)((INT8)(rRaw.au1Data[3]))));
                    dfb_input_dispatch(data->device, &event);
                }
            }
            ...
            D_DEBUG_AT(RCInput, "IR_MOUSE_Info: event.button: 0x%0x02x\n", event.buttion);
            dfb_input_dispatch(data->device, &event);
...
#endif
            printf_display(IsDebugOpen, "[mtrc]KB_%s: code 0x%8x, symbol 0x%8x\n", ((event.type == DIET_KEYPRESS) ? "DOWN" : "UP"), event.key_code,
                event.key_symbol);
            D_DEBUG_AT(RCInput, "[mtrc]KB_%s: code 0x%8x, symbol 0x%8x\n", ((event.type == DIET_KEYPRESS) ? "DOWN" : "UP"), event.key_code,
                event.key_symbol);
            dfb_input_dispatch(data->device, &event);
        } else {
#endif
            ...
            if (event.key_symbol & KEY_SRC_FRONT_PANEL) {
                ...
                if ((rRaw.u1Protocol == MTIRRX_RAW_DATA_RC5) || (rRaw.u1Protocol == MTIRRX_RAW_DATA_RC6)
                    || (rRaw.u1Protocol == MTIRRX_RAW_DATA_NEC)) {
                    event.flags = DIEF_KEYSYMBOL | DIEF_KEYCODE;
                } else {
                    event.flags = DIEF_KEYSYMBOL;
                }
                ...
                event.key_code = (event.key_code) | 0x03000000;
                    ...
                    if (fd == -1) {
                        D_PERROR("[mtrc]write to fifo error\n");
                    }
                }
#endif
                else {
            ...
            printf_display(IsDebugOpen, "[mtrc]RAW: 0x%x, BTN_%s: code 0x%8x, symbol 0x%8x\n", raw, ((event.type == DIET_KEYPRESS) ? "DOWN" : "UP"),
                event.key_code, event.key_symbol);
            D_DEBUG_AT(RCInput, "[mtrc]RAW: 0x%x, BTN_%s: code 0x%8x, symbol 0x%8x\n", raw, ((event.type == DIET_KEYPRESS) ? "DOWN" : "UP"),
                event.key_code, event.key_symbol);
            /*
 ...
 */
static int driver_get_available() { return 1; }
...
static void driver_get_info(InputDriverInfo* info)
{
    ...
    snprintf(info->name, DFB_INPUT_DRIVER_INFO_NAME_LENGTH, "RC Input Driver");
    snprintf(info->vendor, DFB_INPUT_DRIVER_INFO_VENDOR_LENGTH, "Denis Oliver Kropp");

    info->version.major = 0;
    info->version.minor = 1;
}

static INT32 _IrDispatch(void* pvArg) { MTGPIO_CB_T* prGpioCbArg; }
...
static DFBResult driver_open_device(CoreInputDevice* device, unsigned int number, InputDeviceInfo* info, void** driver_data)
{
    ...
    D_DEBUG_AT(RCInput, "[DirectFB]: Wait sometime to create IR...!\n");
    //    usleep( 2000000 );
 ...
 */
static DFBResult driver_get_keymap_entry(CoreInputDevice* device, void* driver_data, DFBInputDeviceKeymapEntry* entry) { return DFB_UNSUPPORTED; }
