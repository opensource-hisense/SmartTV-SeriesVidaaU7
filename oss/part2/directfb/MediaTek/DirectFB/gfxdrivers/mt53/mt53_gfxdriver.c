...
bool
mt53Blit_GLES2( void                       *driver_data,
                 void                       *device_data,
                 CardState                  *state,
                 DFBRectangle               *srect,
                 DFBRectangle               *drect )
{
    MT53DeviceData *sdev = device_data;
    ...
    memset(&blitInfo, 0, sizeof(GLESBlitInfo));
    ...
    blitInfo.eglImageInfo.src.fd     = state->src.dma_fd;
    ...
    blitInfo.eglImageInfo.dst.fd     = state->dst.dma_fd;
    ...
    // gles render.
    ret = sdrv->glesFunc.GLES2Blit( &blitInfo);

    return ret;
}

void
mt53GLES2_Finish( void                       *driver_data,
                  void                       *device_data)
{
     MT53DeviceData *sdev = device_data;
     MT53DriverData *sdrv = driver_data;

     sdrv->glesFunc.GLES2Finish();
}
                    ...
                    CoreDFB             *core )
{
     ...
     funcs->FlushReadCache = mt53FlushReadCache;                /* Invalidate DCache*/
     funcs->FlushTextureCache = mt53FlushTextureCache;          /* Flush DCache */

#ifdef DFB_OGLES_SUPPORT
     if (dfb_config->enable_gles)
     {
         bool ret = GLES_Func_init(&mdrv->glesFunc);
         if (ret == false)
         {
             D_ERROR("EGL/GL Init Failed!\n");
         }
         else {
             if (mdrv->glesFunc.GLES2Blit)
             {
                 funcs->BlitGLES2 = mt53Blit_GLES2;
                 funcs->FinishGLES2 = mt53GLES2_Finish;
             }
         }
     }
#endif
     ...
     mdrv->layer_vdp = dfb_layers_register( mdrv->screen, driver_data, &mt53PrimaryLayerFuncs );
#else
     mdrv->layer_vdp = dfb_layers_register( mdrv->screen, driver_data, &mt53VdpLayerFuncs );
#endif     
     mdrv->cursor = dfb_layers_register( mdrv->screen, driver_data, &mt53PrimaryLayerFuncs );
	 mdrv->layer_vdp2 = dfb_layers_register( mdrv->screen, driver_data, &mt53VdpLayerFuncs );
#ifdef CC_B2R44K2K_SUPPORT
	 mdrv->m_ContainerLayer = dfb_layers_register( mdrv->screen, driver_data, &mt53PrimaryLayerFuncs );
#endif	 
