...
D_DEBUG_DOMAIN( MT53_Cache, "MT53/Cache", "MT53 CPU cache manage" );
...
static bool mt53DrawLines         ( void *drv, void *dev, DFBRegion *line );
static bool mt53DrawRectangleBlend( void* drv, void* dev, DFBRectangle* rect );
static bool mt53DrawRectangle ( void* drv, void* dev, DFBRectangle *rect );
...
void mt53_gfx_set_mmu(GFX_MMU_T *pt_this, int fgPhyEn)
{
    if(DFB_SYSTEMONLY && (fgPhyEn==0))
    {
        pt_this->u4_init         =0x1;
#ifndef GFX_IOMMU_IOVA_SUPPORT
        pt_this->u4_enable       =0x1;
#endif
        pt_this->u4_op_md        =0x0;
#ifdef GFX_IOMMU_IOVA_SUPPORT
        pt_this->u4_src_rw_mmu   =(((pt_this->fg_src_addr_type==1)||(pt_this->fg_src_addr_type==2))?1:0);
        pt_this->u4_dst_rw_mmu   =(((pt_this->fg_dst_addr_type==1)||(pt_this->fg_dst_addr_type==2))?1:0);
        pt_this->u4_enable       = ((pt_this->u4_src_rw_mmu==0)&&(pt_this->u4_dst_rw_mmu==0))?0:1;
#else
...
void mt53_gfx_mmu_save_src_use_addr_type(GFX_MMU_T *pt_this, unsigned int fg_addr_type)
{
    pt_this->fg_src_addr_type=fg_addr_type;
}

void mt53_gfx_mmu_save_dst_use_addr_type(GFX_MMU_T *pt_this, unsigned int fg_addr_type)
{
    pt_this->fg_dst_addr_type=fg_addr_type;
}

#ifdef GFX_IOMMU_IOVA_SUPPORT//need to be kept.
void mt53_gfx_mmu_save_pgd(GFX_MMU_T *pt_this, unsigned int pgd)
{
    if(pgd!=0)
        pt_this->u4_pgt=pgd;
}
...
/*
bSrcOrDst : 0 for src, 1 for dst;
return value: 0 for pa,  2 for iova;
*/
unsigned int mt53_GetIOVAAndPgd(CardState *state, MT53DeviceData *mdev,unsigned char bSrcOrDst, unsigned int * p_iova, unsigned int * p_pgd)
{
    CoreSurfaceAllocation   *dst_allocation ;
    MT53AllocationData *dst_mt53_alloc ;
    SharedAllocationData *dst_shared_alloc;
    CoreSurfaceAllocation   *src_allocation;
    MT53AllocationData *src_mt53_alloc;
    SharedAllocationData *src_shared_alloc;
    unsigned int ret = 0;

    if(state == NULL)
        return 0xff;
       dst_allocation = state->dst.allocation;

    if(dst_allocation == NULL)
        return 0xfe;

     dst_mt53_alloc = (MT53AllocationData *)dst_allocation->data;
     dst_shared_alloc = (SharedAllocationData *)dst_allocation->data;
     if(bSrcOrDst==0)
     {
         src_allocation = state->src.allocation;
         src_mt53_alloc = (MT53AllocationData *)src_allocation->data;
         src_shared_alloc = (SharedAllocationData *)src_allocation->data;
     }

    unsigned int iova = 0xffffffff;
    unsigned int pgd = 0xffffffff;
    unsigned char i = 0;

    // src
    if(bSrcOrDst == 0)
    {
        for(i = 0; i<E_DSSP_MAX; i++)
        {
            if(dfb_mt53->shared->pool==src_allocation->pool)
            {
                iova=src_mt53_alloc->u4_phy;
                ret = 0;
                pgd = 0;
                break;
            }
            else // from shared surface pool
            {
                if(src_shared_alloc->pid[i] == getpid())
                {
                    iova = src_shared_alloc->pin_range[i].iova;//fix me
                    pgd = src_shared_alloc->pin_range[i].pgd;
                    ret = 2;
                    break;
                }
            }
    ...
    else if(bSrcOrDst == 1)
    {
        for(i = 0; i<E_DSSP_MAX; i++)
        {
            if(dfb_mt53->shared->pool==dst_allocation->pool)
            {
                iova = dst_mt53_alloc->u4_phy;
            ...
            {
                if(dst_shared_alloc->pid[i] == getpid())
                {
                    iova = dst_shared_alloc->pin_range[i].iova;//fix me
                    pgd = dst_shared_alloc->pin_range[i].pgd;
                    ret = 2;
                    break;
                }
            }
    ...
    D_DEBUG_AT( MT53_2D, "%s: -> iova=%p, pgd=%p, ret = %d, pid=%d...\n", __func__, iova, pgd, ret, getpid() );
    *p_iova=iova;
    *p_pgd=pgd;
    return ret;
}
...
#ifdef GFX_IOMMU_IOVA_SUPPORT//need to be kept.
        addr_type = mt53_GetIOVAAndPgd(state, mdev, 1, &iova, &pgd);// maybe iova;
        mdev->dst_addr   = (void *)iova;
        mt53_gfx_mmu_save_pgd((GFX_MMU_T *)&mdev->mmu, pgd);
#endif
    }
    else
    {
        addr_type = 0;//pa
        mdev->dst_addr = (void *)state->dst.phys;
    }

    mt53_gfx_mmu_save_dst_use_addr_type((GFX_MMU_T *)&mdev->mmu, addr_type);

    /* Remember destination parameters for usage in rendering functions. */
#ifdef DFB_IOMMU_PRECHECK
    CalcPhysicalAddress((unsigned int)state->dst.addr, 0);
#endif
#ifndef GFX_IOMMU_IOVA_SUPPORT
    mdev->dst_addr   = state->dst.addr;
#endif
                      ...
                      CardState      *state )
{
...
#ifdef GFX_IOMMU_IOVA_SUPPORT//need to be kept.
        addr_type = mt53_GetIOVAAndPgd(state, mdev, 0, &iova, &pgd);// maybe iova;
        mdev->src_addr   = (void *)iova;
        mt53_gfx_mmu_save_pgd((GFX_MMU_T *)&mdev->mmu, pgd);
#endif
    }
    else
    {
        addr_type = 0;//pa
        mdev->src_addr = (void *)state->src.phys;
    }
    
    mt53_gfx_mmu_save_src_use_addr_type((GFX_MMU_T *)&mdev->mmu, addr_type);

    /* Remember source parameters for usage in rendering functions. */
#ifdef DFB_IOMMU_PRECHECK
    CalcPhysicalAddress((unsigned int)state->src.addr, 0);
#endif
#ifndef GFX_IOMMU_IOVA_SUPPORT
    mdev->src_addr   = state->src.addr;
#endif
...
/**
 *  This API was to operate the cache for DFB.
 *  1. After the memory has been written by the CPU(e.g. texture/fill span etc. SW decode), data alwasy havn't writen to DRAM.
 *     You must make sure that HW can ask the whole data, 
 *  2. Also after the memory has been write by the HW(e.g. fill rect or blit etc. HW decode), data always whole written to DRAM.
 *     If in this case, the buffer would operate by CPU, but cpu would ask from dcache fisrt, to avoid this case, must invalidate dcache.
 * 
 */
static inline void mt53DCacheIoctl( MT53_CACHE_DATA* cache)
{
    if (cache == NULL)
    {
        return ;
    ...
    D_DEBUG_AT( MT53_Cache,"MediaTek/Driver: %s [(SRC)falg: 0x%x, addr: 0x%x, size: 0x%x] [(DST)flag: 0x%x, addr: 0x%x, size: 0x%x]\n", 
                __func__,
                cache->u4_src_fg,
                cache->u4_src_addr,
                cache->u4_src_size,
    ...
    if ( ioctl(dfb_mt53->fd, FBIO_GFX_FLUSH_DCACHE, cache) < 0 )
    {
        D_PERROR( "MediaTek/Driver: %s failed!\n"
                  "[(SRC)falg: 0x%x, addr: 0x%x, size: 0x%x]\n"
                  "[(DST)flag: 0x%x, addr: 0x%x, size: 0x%x]\n",
                  __func__,
                  cache->u4_src_fg,
                  cache->u4_src_addr,
                  cache->u4_src_size,
 ...
 */
void mt53FlushTextureCache( void *drv, void *dev, CoreSurfaceBufferLock *lock )
{
    ...
    bzero( &cache, sizeof(MT53_CACHE_DATA) );

    cache.u4_src_addr       = (unsigned int)lock->addr;
    cache.u4_src_cpuva      = cache.u4_src_addr;
    cache.u4_src_pa         = lock->phys;
    cache.u4_src_size       = lock->size;
    cache.u4_src_fg         = DFB_GFX_FLUSH_DCACHE;

    mt53DCacheIoctl(&cache);
 ...
 */
void mt53FlushReadCache( void *drv, void *dev, CoreSurfaceBufferLock *lock )
{
    MT53DeviceData *mdev = (MT53DeviceData *) dev;
    MT53DriverData *mdrv = (MT53DriverData *) drv;
    MT53_CACHE_DATA  cache;
    ...
    cache.u4_src_pa         = lock->phys;
    cache.u4_src_size       = lock->size;
    cache.u4_src_fg         = DFB_GFX_INVALID_DCACHE;
 ...
 * of a texture) make sure the accelerator won't use cached texture data
 */
void mt53FlushInvalidCache( unsigned int u4_addr, unsigned int u4_size  )
{
    MT53_CACHE_DATA cache;
    ...
    cache.u4_src_fg        = DFB_GFX_FLUSH_INVALID_DCACHE;
    cache.u4_src_addr      = u4_addr;
    cache.u4_src_size      = u4_size;
    cache.fg_src_addr_type = 1;
...
mt53EngineSync( void *drv, void *dev )
{
    ...
    DIRECT_INTERFACE_DBG_DELTA_END(DDDT_GFX_SYNC);
...
void mt53EmitCommands( void *drv, void *dev )
{
    ...
    mt53_gfx_set_mmu((GFX_MMU_T *)&mdev->mmu, 0);

    if(ioctl(mdrv->fd, FBIO_GFX_LOCK,&mdev->mmu) < 0)
    {
    ...
    if(ioctl( mdrv->fd, FBIO_GFX_FLUSH,NULL) < 0)
    {
        D_PERROR( "MediaTek/Driver: FBIO_GFX_FLUSH failed!\n" );
    }

    memset(&mdev->mmu,0x0,sizeof(GFX_MMU_T));

    DIRECT_INTERFACE_DBG_DELTA_END(DDDT_EMMT_CMD);

    if(DIRECT_INTERFACE_DBG_DELTA)
    {
        if(!dfb_interface_dbg_delta_enable())
        {
            if(mt_get_dbg_info(2)&0x1)
            {
               dfb_interface_dbg_set_delta_enable(true);
            }
        }

        if(dfb_interface_dbg_delta_enable())
        {
            if(mt_get_dbg_info(2)&0x2)
            {
                dfb_interface_dbg_set_delta_enable(false);
                dfb_dbg_delta_print();
                dfb_dbg_delta_clear();
            }
        }
                ...
                DFBAccelerationMask  accel )
{
    ...
    if(CC_DFB_DEBUG_FG& CC_DFB_DEBUG_SW_ONLY)
    {
        if(mt_get_dbg_info(E_DFB_DBG_INFO_3)& CC_DFB_DEBUG_SW_ONLY)
        {
             return;
        }
    }
...
/*check if hardware support this operation*/
    ...
    case DSPF_NV16:
        #ifdef CC_B2R44K2K_SUPPORT
        if(( (accel != DFXL_STRETCHBLIT) && (!(state->blittingflags & (DSBLIT_ROTATE90 | DSBLIT_ROTATE180 | DSBLIT_ROTATE270 | DSBLIT_FLIP_HORIZONTAL)))))
        {
            #ifdef CC_GFX_LOG
    ...
    else    //bitblt functions
    {
        ...
        case DSPF_LUT8:
            if (state->destination->config.format !=DSPF_LUT8)
            {
               ...
               case DSPF_NV16:
                 /* Check for rotation */
                   if(state->destination->config.format != DSPF_NV16 || !(state->blittingflags & (DSBLIT_ROTATE90 | DSBLIT_ROTATE180 | DSBLIT_ROTATE270 | DSBLIT_FLIP_HORIZONTAL)))
                   {
                    //printf("  Unsupport src/dst format(0x%x)\n", state->destination->config.format);
              ...
              DFBAccelerationMask  accel )
{
    ...
    mdev->porterduff = _checkAlphaComp(state->src_blend, state->dst_blend);
     ...
     if(state->blittingflags & DSBLIT_FLIP_HORIZONTAL)
     {
         mdev->rotateopt |= MT53FB_VDP_ROTATE_FLIP;
     }
#endif
    ...
    case DFXL_DRAWRECTANGLE:
        ...
        if(state->drawingflags & DSDRAW_BLEND)
        {
            ...
            {
                if(state->drawingflags & (DSDRAW_DST_PREMULTIPLY | DSDRAW_DEMULTIPLY))
                     _u4PremultDst = 0;
                else
                     _u4PremultDst = 1;
            }
            ...
            funcs->DrawRectangle = mt53DrawRectangleBlend;
        }
        else
        {
            if (state->drawingflags & DSDRAW_SRC_PREMULTIPLY)
            ...
            funcs->DrawLine      = mt53DrawLines;
            funcs->DrawRectangle = mt53DrawRectangle;
        }
    ...
    case DFXL_STRETCHBLIT:
...
#ifdef CC_DFB_SUPPORT_VDP_LAYER
       else if(state->blittingflags & (DSBLIT_ROTATE90 | DSBLIT_ROTATE180 | DSBLIT_ROTATE270 | DSBLIT_FLIP_HORIZONTAL))
               {
        ...
        #ifdef CC_B2R44K2K_SUPPORT
        if(state->destination && (state->destination->config.caps & DSCAPS_FROM_MPEG)
            && state->source && (state->source->config.caps & DSCAPS_FROM_MPEG))
        {
            funcs->StretchBlit = mt53B2RStretchBlit;
        }
        #endif
        break;
    ...
    if(CC_DFB_DEBUG_FG& CC_DFB_DEBUG_CARD_CHECK)
    {
        if(0) //(mt_get_dbg_info(E_DFB_DBG_INFO_3)& CC_DFB_DEBUG_CARD_CHECK)
        {
            D_INFO("%s[%d][0x%x,0x%x][0x%x,0x%x]\n",
                __FUNCTION__,           dfb_gfxcard_get_num(),
                (int)state->source,     (int)state->destination,
                accel,                  state->blittingflags);
        }
    }
...
static bool
mt53DrawLines( void* drv, void* dev, DFBRegion *line )
{
    MT53DeviceData* mdev = (MT53DeviceData*)dev;
    MT53DriverData* mdrv = (MT53DriverData*)drv;
    int i, x1, y1, x2, y2;

    D_DEBUG_AT( MT53_2D, "%s (%d,%d - %d,%d)\n", __FUNCTION__, DFB_REGION_VALS( line ) );
    ...
    /* Draw vertical line ?  */
    if ( x1 == x2  ) {
        GFX_VLine( x1, y1, (y1 >= y2 ? (y1 - y2) : (y2 - y1)) );
    }

    /* Draw horizontal line ? */
    else if ( y1 == y2 ) {
        GFX_HLine( x1, y1, (x1 > x2 ? (x1 - x2) : (x2 - x1)) );
    }
...
static bool
mt53DrawRectangleBlend( void* drv, void* dev, DFBRectangle* rect )
{
    MT53DeviceData* mdev = (MT53DeviceData*)dev;
    MT53DriverData* mdrv = (MT53DriverData*)drv;
    ...
    GFX_ComposeLoop( 0, 0, x, y, w, 1, 0xff, porterduff - 1, 1 );
    GFX_ComposeLoop( 0, 0, x, y, 1, h, 0xff, porterduff - 1, 1 );
    GFX_ComposeLoop( 0, 0, x, y + h, w, 1, 0xff, porterduff - 1, 1 );
    GFX_ComposeLoop( 0, 0, x + w, y, 1, h, 0xff, porterduff - 1, 1 );
...
static bool
mt53DrawRectangle( void* drv, void* dev, DFBRectangle *rect )
{
    MT53DeviceData* mdev = (MT53DeviceData*)dev;
    MT53DriverData* mdrv = (MT53DriverData*)drv;
    int x, y, w, h;
...
//  MT53DriverData *mdrv = (MT53DriverData*) drv;
    ...
    //if( MTVGfx_JugRect(*(MTVGFX_QUADRILATERAL_SET_T *)&(_SetVgfxScaler->_SrcQuard)))
    //{
    //    return false;
   // }
   // else
    ...
    //_bMatrix = MTVGfx_JugRect(*(MTVGFX_QUADRILATERAL_SET_T *)&(_SetVgfxScaler->_DstQuard));
    if( _bMatrix )
    {
...
mt53StretchBlit( void *drv, void *dev, DFBRectangle *srect, DFBRectangle *drect )
{
    ...
    if(0) /*(mt_get_dbg_info(0x2)&CC_DFB_DEBUG_CARD_SBLIT)*/
    {
		D_INFO("%s:Clip(%d,%d,%d,%d)",__FUNCTION__, prClip->x1, prClip->y1, prClip->x2, prClip->y2);
		D_INFO("(%d,%d,%d,%d)", srect->x, srect->y, srect->w, srect->h);
    ...
    {
#ifdef GFX_IOMMU_IOVA_SUPPORT
        memcpy(&rStretchBlit.mmu, &(mdev->mmu), sizeof(GFX_MMU_T));
#endif
        mt53_gfx_set_mmu(&rStretchBlit.mmu, 0);
        //MT53_ST_SET_DCACHE_INFO();
    ...
    DIRECT_INTERFACE_DBG_DELTA_END(DDDT_IMGRZ);
    return true;
}
...
static bool mt53StretchBlitBlend( void *drv, void *dev, DFBRectangle *srect, DFBRectangle *drect )
{
    ...
    if ((drect->w * drect->h) <= (TMPBUF_WIDTH*TMPBUF_HEIGHT))
    {
        ...
        D_DEBUG_AT( MT53_2D, "%s:Clip(%d,%d,%d,%d) src(%d,%d,%d,%d) dst(%d,%d,%d,%d)\n",
                    __FUNCTION__, prClip->x1, prClip->y1, prClip->x2, prClip->y2,
            ...
            (mt_get_render_option()&&(_pCardState->render_options&(DSRO_SMOOTH_UPSCALE|DSRO_SMOOTH_DOWNSCALE)))))
        {
            ...
            rStretchBlit.u4PremultSrcR      = _u4PremultSrc;
            rStretchBlit.u4PremultDstR      = _u4PremultDst;
            rStretchBlit.u4PremultDstW      = _u4PremultWDst;
