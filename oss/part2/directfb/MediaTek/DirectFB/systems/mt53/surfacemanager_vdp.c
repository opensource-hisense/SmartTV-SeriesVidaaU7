...
DFBResult
dfb_surfacemanager_vdp_create( CoreDFB         *core,
                           unsigned int     length,
...
void
dfb_surfacemanager_vdp_destroy( SurfaceManager *manager )
{
     Chunk *chunk;
     void  *next;
...
DFBResult dfb_surfacemanager_vdp_adjust_heap_offset( SurfaceManager *manager,
                                                 int             offset )
{
     D_MAGIC_ASSERT( manager, SurfaceManager );
     D_ASSERT( offset >= 0 );
...
DFBResult dfb_surfacemanager_vdp_allocate( CoreDFB            *core,
                                       SurfaceManager     *manager,
                                       CoreSurfaceBuffer  *buffer,
                                       Chunk             **ret_chunk )                                  
{
...
//NULL means check only     D_ASSERT( ret_chunk != NULL );
     ...
     dfb_gfxcard_calc_buffer_size( device, buffer, &pitch, &length,DSCAPS_FROM_JPEG_VDP );
...
*/
     direct_log_printf( NULL, "  -> failed (%d/%d avail)\n", manager->avail, manager->length );

     /* no luck */
     return DFB_NOVIDEOMEMORY;
}

DFBResult dfb_surfacemanager_vdp_deallocate( SurfaceManager *manager,
                                         Chunk          *chunk)
{
     CoreSurfaceBuffer *buffer;
