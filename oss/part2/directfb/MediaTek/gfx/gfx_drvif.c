...
MT53_GFX_OP_DATA t_gfx_op_data;

#define GFX_OP_SET_INFO(U4_DST,U4_DST_SIZE,U4_SRC,U4_SRC_SIZE)\
    t_gfx_op_data.u4_dst_addr = U4_DST;\
    t_gfx_op_data.u4_dst_size = U4_DST_SIZE;\
    t_gfx_op_data.u4_src_addr = U4_SRC;\
    t_gfx_op_data.u4_src_size = U4_SRC_SIZE;\
...
UINT32 u4_gfx_op_dst_addr =0x0;
UINT32 u4_gfx_op_dst_size =0x0;
UINT32 u4_gfx_op_src_addr =0x0;
UINT32 u4_gfx_op_src_size =0x0;
...
void _GFX_Fill(UINT32 u4DstX, UINT32 u4DstY, UINT32 u4Width, UINT32 u4Height)
{
    ...
    GFX_OP_SET_INFO(ui4_dst_addr,((_u4DstPitch*u4Height)-(u4DstX << _u4DstShift)),0x0,0x0);
...
void _GFX_HLine(UINT32 u4DstX, UINT32 u4DstY, UINT32 u4Width)
{
    ...
    ui4_dst_addr = (UINT32)(
            _pu1DstBase + (u4DstY * _u4DstPitch) + (u4DstX << _u4DstShift));
    GFX_OP_SET_INFO(ui4_dst_addr,(_u4DstPitch*1),0x0,0x0);
        ...
        UINT32 u4Width, UINT32 u4Height)
{
    ...
    GFX_OP_SET_INFO(ui4_dst_addr,((_u4DstPitch*u4Height)-(u4DstX << _u4DstShift)),ui4_src_addr,((_u4SrcPitch*u4Height)-(u4SrcX << _u4SrcShift)));
...
void GFX_Get_Dcache_Info(UINT32 u4SrcX, UINT32 u4SrcY, UINT32 u4SrcW,UINT32 u4SrcH,
    UINT32 u4DstX, UINT32 u4DstY, UINT32 u4DstW,UINT32 u4DstH,MT53_GFX_OP_DATA* pt_this)
{
    UINT32 u4MnV, u4MnH;
    UINT32 ui4_dst_addr = 0x0;
    UINT32 ui4_src_addr = 0x0;
    ...
    pt_this->u4_dst_addr = ui4_dst_addr;
    pt_this->u4_dst_size = ((_u4DstPitch*u4DstH)-(u4DstX << _u4DstShift));
    pt_this->u4_src_addr = ui4_src_addr;
    pt_this->u4_src_size = ((_u4SrcPitch*u4SrcH)-(u4SrcX << _u4SrcShift));
}
        ...
        UINT32 u4DstH)
{
    ...
    ui4_src_addr = (UINT32)(
            _pu1SrcBase + (u4SrcY * _u4SrcPitch) + (u4SrcX << _u4SrcShift));
    GFX_OP_SET_INFO(ui4_dst_addr,(_u4DstPitch*u4DstH) - (u4DstX << _u4DstShift),ui4_src_addr,(_u4SrcPitch*u4SrcH)-(u4SrcX << _u4SrcShift));
    REG(fg_SRC_BSAD) = ui4_src_addr;
    REG(fg_SRC_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_src_addr);
    REG(fg_DST_BSAD) = ui4_dst_addr;
    REG(fg_DST_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_dst_addr);
        ...
        UINT32 u4OpCode, UINT32 u4RectSrc)
{
    ...
    if(((CM_ARGB4444_DIRECT16 == _u4SrcCM)||(CM_ARGB1555_DIRECT16 == _u4SrcCM)||(CM_RGB565_DIRECT16 == _u4SrcCM))&&
        ((((ui4_src_addr&0xf)*8)+(u4Width*16)) <= 128)&&(u4CMDiff))
    {
        REG(fg_SRCOWN) = 0x1;
    }
    else
        ...
        UINT32 u4Height)
{
    ...
    GFX_OP_SET_INFO(ui4_dst_addr,((_u4DstPitch*u4Height)-(u4DstX << _u4DstShift)),ui4_src_addr,(_u4SrcPitch*u4Height));
    REG(fg_SRC_BSAD) = ui4_src_addr;
    REG(fg_SRC_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_src_addr);
    REG(fg_DST_BSAD) = ui4_dst_addr;
    REG(fg_DST_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_dst_addr);
        ...
        UINT32 u4DstY, UINT32 u4HoriLineWidth)
{
    ...
    ui4_src_addr = (UINT32)(
            _pu1SrcBase + (u4SrcY * _u4SrcPitch) + (u4SrcX << _u4SrcShift));
    GFX_OP_SET_INFO(ui4_dst_addr,(_u4DstPitch*1),ui4_src_addr,(_u4SrcPitch*1));
    REG(fg_SRC_BSAD) = ui4_src_addr;
    REG(fg_SRC_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_src_addr);
    REG(fg_DST_BSAD) = ui4_dst_addr;
    REG(fg_DST_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_dst_addr);
        ...
        UINT32 u4Mode)
{
    ...
    GFX_OP_SET_INFO(ui4_dst_addr,(_u4DstPitch*u4Height) - (u4X << _u4DstShift),0x0,0x0);
    REG(fg_DST_BSAD) = ui4_dst_addr;
    REG(fg_DST_BSAD_MSB) = GFX_REG_ADDR_MSG(ui4_dst_addr);
