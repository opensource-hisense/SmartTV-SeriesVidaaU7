	...
	if (dev) {
		if (msg.listen_channel) {
            ...
            if (msg.wfd_subelems) {
                ...
                if(msg.wfd_assoc_bssid)
                {
                    dev->info.include_wfd_assoc_bssid = 1;
                    os_memcpy(dev->info.wfd_assoc_bssid, msg.wfd_assoc_bssid,
                      sizeof(dev->info.wfd_assoc_bssid));
                }
#endif
            }
            wpa_printf(MSG_DEBUG, "P2P: Update device entry based on Probe Req: " MACSTR
...
#ifdef CONFIG_MTK_HE_SUPPORT
    p2p_dbg(p2p, "%s: sent Ok to " MACSTR, __FUNCTION__, MAC2STR(addr));
#endif
