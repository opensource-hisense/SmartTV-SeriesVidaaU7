			   ...
			   int authorized)
{
	...
	if (authorized) {
        ...
        {
            char wfd_dev_info_hex[WFD_DEV_INFO_HEX_SIZE];
            if (sta->wfd_ie) {
                os_memset(wfd_dev_info_hex, 0, sizeof(wfd_dev_info_hex));
                wpa_snprintf_hex(wfd_dev_info_hex, sizeof(wfd_dev_info_hex),
                    wpabuf_head(sta->wfd_ie),
                    WFD_DEV_INFO_SIZE);
                    wpa_msg(hapd->msg_ctx, MSG_INFO, P2P_EVENT_WFD_INFO MACSTR
                    " wfd_dev_info=0x%s", MAC2STR(sta->addr), wfd_dev_info_hex);
                    /* Send WFD-DEV-INFO event to parent interface as well */
                if (hapd->msg_ctx_parent && hapd->msg_ctx_parent != hapd->msg_ctx)
                    wpa_msg(hapd->msg_ctx_parent, MSG_INFO, P2P_EVENT_WFD_INFO MACSTR
                    " wfd_dev_info=0x%s", MAC2STR(sta->addr), wfd_dev_info_hex);
                }
        }
#endif
