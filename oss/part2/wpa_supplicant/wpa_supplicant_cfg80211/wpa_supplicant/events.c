					  ...
					  int locally_generated)
{
	...
	if (!is_zero_ether_addr(bssid) ||
        ...
        /* Send the disconnected event to parent interface like p2p0 */
        if (wpa_s->parent) {
            wpa_msg(wpa_s->parent, MSG_INFO, WPA_EVENT_DISCONNECTED "bssid=" MACSTR
                " reason=%d%s",
                MAC2STR(bssid), reason_code,
                locally_generated ? " locally_generated=1" : "");
        }
		...
		if (wpa_s->ap_iface == NULL) {
			...
			if (stype == WLAN_FC_STYPE_PROBE_REQ &&
                ...
                wpa_dbg(wpa_s, MSG_EXCESSIVE,
                 "%s:Receive ProbeReq from " MACSTR ,
                __FUNCTION__, MAC2STR(src));
#endif
