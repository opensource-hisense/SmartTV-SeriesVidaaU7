...
{
    char *ptr = NULL;
    char allinfo_tmp[WFD_ALL_INFO_TMP_LEN];

    os_memset(allinfo_tmp, 0, sizeof(allinfo_tmp));
    os_strlcpy(allinfo_tmp, allinfo, sizeof(allinfo_tmp));
    ptr = allinfo_tmp;
...
{
    char allinfo[WFD_ALL_INFO_TMP_LEN];
    ...
    os_snprintf(allinfo, sizeof(allinfo)-1, "0 %04x%04x%04x%04x",
                WFD_ALL_INFO_PRINT_NUM_6, dev_info, ctrl_port, max_tput);
...
{
	if(!bssid || strlen(bssid) != WFD_ALL_INFO_BSSID_LEN)
	{
    ...
    if (os_strcasecmp(cmd, "wfd_bssid") == 0)
    {
		...
		os_snprintf(allinfo, sizeof(allinfo)-1, "1 %04x%s",
					WFD_ALL_INFO_PRINT_NUM_6, bssid);
		wpa_printf(MSG_DEBUG, "wifi_display_subelem_set=[%s]", allinfo);
