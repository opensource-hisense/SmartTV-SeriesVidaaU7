...
#ifdef CONFIG_MTK_HE_SUPPORT
    int res;
    res = os_snprintf(ifname, len, "p2p1");
#else
...
#ifdef CONFIG_MTK_HE_SUPPORT
    /* Send wfd dev info event */
    struct p2p_device *dev;
    char wfd_dev_info_hex[2 * WFD_DEV_INFO_SIZE + 1];

    dev = p2p_get_device(wpa_s->global->p2p, src);
    if (dev && dev->info.wfd_subelems)
    {
        ...
        wpa_snprintf_hex(wfd_dev_info_hex, sizeof(wfd_dev_info_hex),
                    ...
                    WFD_DEV_INFO_SIZE);
        wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_WFD_INFO MACSTR " wfd_dev_info=0x%s",
            MAC2STR(src), wfd_dev_info_hex);
#ifdef CONFIG_SUPPORT_GET_WFD_ASSOCBSSID_IN_CONN_MSG
    if(dev->info.include_wfd_assoc_bssid)
            {
                wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
                    " dev_passwd_id=%u p2p_dev_addr=" MACSTR
                    " name='%s' config_methods=0x%x wfd_dev_info=0x%s wfd_assoc_bssid=" MACSTR, 
                    MAC2STR(src),
                    dev_passwd_id,MAC2STR(dev->info.p2p_device_addr),
                    dev->info.device_name,dev->info.config_methods,wfd_dev_info_hex,MAC2STR(dev->info.wfd_assoc_bssid));
            }
            else
            {
                wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
                    " dev_passwd_id=%u p2p_dev_addr=" MACSTR
                    " name='%s' config_methods=0x%x wfd_dev_info=0x%s", MAC2STR(src),
                    dev_passwd_id,MAC2STR(dev->info.p2p_device_addr),
                    dev->info.device_name,dev->info.config_methods,wfd_dev_info_hex);
            }
        }
        else if(dev)
        {
                wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
                    ...
                    MAC2STR(src),
                    dev_passwd_id,MAC2STR(dev->info.p2p_device_addr),
                    dev->info.device_name,dev->info.config_methods);
#endif /* CONFIG_SUPPORT_GET_WFD_ASSOCBSSID_IN_CONN_MSG */
        }
#endif /* CONFIG_MTK_HE_SUPPORT */
#ifdef CONFIG_SUPPORT_GET_WFD_ASSOCBSSID_IN_CONN_MSG
        else
			       ...
			       size_t group_id_len)
{
    ...
    dev = p2p_get_device(wpa_s->global->p2p, peer);
    if (dev && dev->info.wfd_subelems)
    {
        os_memset(wfd_dev_info_hex, 0, sizeof(wfd_dev_info_hex));
        wpa_snprintf_hex(wfd_dev_info_hex, sizeof(wfd_dev_info_hex),
        ...
        if(dev->info.include_wfd_assoc_bssid)
        {
            res = os_snprintf(params, sizeof(params), " p2p_dev_addr=" MACSTR
                    ...
                    "wfd_dev_info=0x%s wfd_assoc_bssid=" MACSTR,
                    MAC2STR(dev_addr),
                    ...
                    group ? " group=" : "",
                    group ? group->ifname : "",
                    wfd_dev_info_hex,MAC2STR(dev->info.wfd_assoc_bssid));
        ...
        wpa_msg(wpa_s, MSG_INFO,
            P2P_EVENT_WFD_INFO MACSTR " wfd_dev_info=0x%s",
            MAC2STR(peer), wfd_dev_info_hex);
