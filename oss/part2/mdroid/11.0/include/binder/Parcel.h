 ...
 */
// std::enable_if_t in c++ 17 , so change it to std::enable_if STDLIBC_INTEGER
...
#include <type_traits>
#include <vector>
#include <limits> //STDLIBC_INTEGER
...
public:
    ...
    template<typename T, std::enable_if_t<std::is_enum<T>::value && std::is_same<typename std::underlying_type_t<T>,int8_t>::value, bool> = 0>
...
private:
    ...
    template<typename T, std::enable_if_t<std::is_same<typename std::underlying_type_t<T>,int32_t>::value, bool> = 0>
    status_t            writeEnum(const T& val);
    template<typename T, std::enable_if_t<std::is_same<typename std::underlying_type_t<T>,int64_t>::value, bool> = 0>
    status_t            writeEnum(const T& val);
...
template<typename T, std::enable_if_t<std::is_same<typename std::underlying_type_t<T>,int32_t>::value, bool>>
status_t Parcel::writeEnum(const T& val) {
    return writeInt32(static_cast<int32_t>(val));
}
template<typename T, std::enable_if_t<std::is_same<typename std::underlying_type_t<T>,int64_t>::value, bool>>
status_t Parcel::writeEnum(const T& val) {
    return writeInt64(static_cast<int64_t>(val));
}

template<typename T, std::enable_if_t<std:: is_enum<T>::value && std::is_same<typename std::underlying_type_t<T>,int8_t>::value, bool>>
...
status_t Parcel::readEnumVector(std::vector<T>* val) const {
    size_t size;
	status_t status = reserveOutVector(val, &size);
