...
//GGGG txn->pid
    ALOGI("[GGGG]binder_parse/BR_TRANSACTION  code=%d pid=%d uid=%d res=%d\n",
           txn.transaction_data.code, txn.transaction_data.sender_pid, txn.transaction_data.sender_euid,res);
...
static void *bio_get(struct binder_io *bio, size_t size)
{
   ...
   ALOGE("[GGGG] @@@@@@ bio_get line=%d   get bio->data_avail=%d size=%d \n",__LINE__,(int)bio->data_avail,(int)size);
        bio->data_avail = 0;
        bio->flags |= BIO_F_OVERFLOW;
        return NULL;
    }  else {
...
uint16_t *bio_get_string16(struct binder_io *bio, size_t *sz)
{
    ...
    if (sz)
        *sz = len;
 uint16_t * ret = bio_get(bio, (len + 1) * sizeof(uint16_t));

    if(ret == NULL)
   ALOGE("[GGGG]XXXXXXXXXXXXXXX bio_get_string16  get NULL len=%d ",(int)len);
   else if(tmp < 20)
   ALOGE("[GGGG]OOOOOOOOOOOOO bio_get_string16Good len=%d ",(int)len);
