...
#include <log/config_read.h> /* __android_log_config_read_close() definition */
#include <log/config_write.h>
#include <log/log_portability.h>
    ...
    if ((uid != AID_SYSTEM) && (uid != AID_ROOT) && (uid != AID_LOG)) {
      ...
      if ((gid != AID_SYSTEM) && (gid != AID_ROOT) && (gid != AID_LOG)) {
        ...
        if ((gid != AID_SYSTEM) && (gid != AID_ROOT) && (gid != AID_LOG)) {
          ...
          groups = calloc(num_groups, sizeof(gid_t));  //STDLIBC_INTEGER
          if (!groups) {
            return -ENOMEM;
          }
...
static void __android_log_cache_available(struct android_log_transport_write* node) {
  ...
  for (i = LOG_ID_MIN; i < LOG_ID_MAX; ++i) {
    if (node->write && (i != LOG_ID_KERNEL) &&
        ...
        (!node->available || ((*node->available)(i) >= 0))) {    //STDLIBC_INTEGER
      node->logMask |= 1 << i;
    }
  }
}

int __android_log_dev_available() {     //STDLIBC_INTEGER
...
#if defined(__ANDROID__)
  if (m != (EventTagMap*)(uintptr_t)-1LL) {};//android_closeEventTagMap(m);//STDLIBC_INTEGER
#endif
}
...
static int __write_to_log_daemon(log_id_t log_id, struct iovec* vec, size_t nr) {
  ...
  if (log_id == LOG_ID_SECURITY) {
    ...
    if (!m) {
      ...
      if (!m) {
        //m = android_openEventTagMap(NULL);//STDLIBC_INTEGER
    ...
    if (m && (m != (EventTagMap*)(uintptr_t)-1LL)) {
      //tag = android_lookupEventTag_len(m, &len, get4LE(vec[0].iov_base));//STDLIBC_INTEGER
    }
    ret = __android_log_is_loggable_len(ANDROID_LOG_INFO, tag, len, ANDROID_LOG_VERBOSE);
    if (f) { /* local copy marked for close */
      //android_closeEventTagMap(f);//STDLIBC_INTEGER
    }
    if (!ret) {
      errno = save_errno;
      return -EPERM;
    ...
    const char* tag = vec[0].iov_base; //STDLIBC_INTEGER
...
#pragma clang diagnostic pop
  ...
  return write_to_log(bufID, vec, 3);    //STDLIBC_INTEGER
}
