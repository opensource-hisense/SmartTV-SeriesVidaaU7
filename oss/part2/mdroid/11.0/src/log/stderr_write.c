 ...
 */
#undef __ANDROID__ //STDLIBC_INTEGER
#include <errno.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
...
static int stderrOpen() {
  ...
  ctx = calloc(1, sizeof(struct stderrContext));    //STDLIBC_INTEGER
  if (!ctx) {
    return -ENOMEM;
  }
...
static void stderrClose() {
  struct stderrContext* ctx = stderrLoggerWrite.context.priv;    //STDLIBC_INTEGER
