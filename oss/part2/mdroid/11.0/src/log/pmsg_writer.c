...
#include <sys/syscall.h>//STDLIBC_INTEGER
#include <log/log_properties.h>
#include <private/android_filesystem_config.h>
#include <private/android_logger.h>

#include <log/config_write.h>  //STDLIBC_INTEGER
#include <log/log_portability.h>  //STDLIBC_INTEGER
#include <log/logger.h>  //STDLIBC_INTEGER
#include <log/uio.h> //STDLIBC_INTEGER
#define SNET_EVENT_LOG_TAG 1397638484  // STDLIBC_INTEGER
#define gettid() syscall(__NR_gettid)  // STDLIBC_INTEGER
static int pmsgOpen();
...
static int pmsgWrite(log_id_t logId, struct timespec* ts, struct iovec* vec, size_t nr) {
  ...
  if ((logId == LOG_ID_EVENTS) && !__android_log_is_debuggable()) {
    ...
    if (SNET_EVENT_LOG_TAG != get4LE(vec[0].iov_base)) {    //STDLIBC_INTEGER
      return -EPERM;
    }
  }
