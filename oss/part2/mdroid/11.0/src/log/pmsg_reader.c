...
#include <string.h>
#include <sys/types.h>
#include <limits.h>//STDLIBC_INTEGER
#include <private/android_filesystem_config.h>
#include <private/android_logger.h>

#include <log/config_read.h>//STDLIBC_INTEGER
#include <log/logger.h>//STDLIBC_INTEGER
...
ssize_t __android_log_pmsg_file_read(log_id_t logId, char prio, const char* prefix,
  ...
  while (pmsgRead(&logger_list, &transp, &transp.logMsg) > 0) {
    ...
    if (node == &name_list) {
      ...
      names = calloc(1, sizeof(*names) + len);    //STDLIBC_INTEGER
      if (!names) {
        ret = -ENOMEM;
        break;
      }
      strcpy(names->name, msg + sizeof(prio));
      names->id = transp.logMsg.entry.lid;    //STDLIBC_INTEGER
      names->prio = *msg;
    ...
    /* Add content */
    content =
        calloc(1, sizeof(content->node) + hdr_size + transp.logMsg.entry.len);   //STDLIBC_INTEGER
    if (!content) {
      ret = -ENOMEM;
      break;
    }
  ...
  list_for_each_safe(node, n, &name_list) {
    ...
    list_for_each_safe(content_node, m, &names->content) {
      ...
      if (!buf) {
        buf = malloc(sizeof(char));    //STDLIBC_INTEGER
        if (!buf) {
          ret = -ENOMEM;
          list_remove(content_node);
          free(content);
      ...
      while (sequence < content->entry.nsec) {
        ...
        buf = realloc(buf, len + sizeof(char) + sizeof(char));    //STDLIBC_INTEGER
        if (!buf) {
          break;
        }
      ...
      /* plus space for enforced nul */
      buf = realloc(buf, len + add_len + sizeof(char));    //STDLIBC_INTEGER
      if (!buf) {
        ret = -ENOMEM;
        list_remove(content_node);
        free(content);
