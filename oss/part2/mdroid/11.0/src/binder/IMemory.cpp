...
#include <log/log_main.h> //STDLIBC_INTER
#define VERBOSE   0
...
private:
    ...
    mutable volatile int32_t mHeapId;//STDLIBC_INTEGER
    mutable void*       mBase;
...
BpMemoryHeap::~BpMemoryHeap() {
    if (mHeapId != -1) {//STDLIBC_INTEGER
        close(mHeapId);//STDLIBC_INTEGER
        if (mRealHeap) {
            ...
            if (mBase != MAP_FAILED) {
                ...
                if (VERBOSE) {
                    ALOGD("UNMAPPING binder=%p, heap=%p, size=%zu, fd=%d",
                            binder.get(), this, mSize, mHeapId/*STDLIBC_INTEGER*/);
                }
    ...
    if (mHeapId == -1) {//STDLIBC_INTEGER
        ...
        if (heap->mBase != MAP_FAILED) {
            ...
            if (mHeapId == -1) {//STDLIBC_INTEGER
                ...
                android_atomic_write( dup( heap->mHeapId ), &mHeapId );//STDLIBC_INTEGER
            }
        } else {
...
void BpMemoryHeap::assertReallyMapped() const
{
    if (mHeapId == -1) {//STDLIBC_INTEGER
        ...
        Mutex::Autolock _l(mLock);
        if (mHeapId == -1) {//STDLIBC_INTEGER
            int fd = dup( parcel_fd );//STDLIBC_INTEGER
            ...
            if (mBase == MAP_FAILED) {
                ...
                mFlags = flags;
                mOffset = offset;
                android_atomic_write(fd, &mHeapId);//STDLIBC_INTEGER
            }
        }
    }
}
...
int BpMemoryHeap::getHeapID() const {
    assertMapped();
    return mHeapId;//STDLIBC_INTEGER
}
...
sp<IMemoryHeap> HeapCache::find_heap(const sp<IBinder>& binder)
{
    ...
    if (i>=0) {
        ...
        ALOGD_IF(VERBOSE,
                ...
                static_cast<BpMemoryHeap*>(info.heap.get())->mHeapId,////STDLIBC_INTEGER
                info.count);
        ++info.count;
        return info.heap;
    } else {
...
void HeapCache::free_heap(const wp<IBinder>& binder)
{
    sp<IMemoryHeap> rel;
    {
        ...
        if (i>=0) {
            heap_info_t& info(mHeapCache.editValueAt(i));
            int32_t c = android_atomic_dec(&info.count);//STDLIBC_INTEGER
            if (c == 1) {//STDLIBC_INTEGER
                ALOGD_IF(VERBOSE,
...
void HeapCache::dump_heaps()
{
    ...
    for (int i=0 ; i<c ; i++) {
        ...
        ALOGD("hey=%p, heap=%p, count=%d, (fd=%d, base=%p, size=%zu)",
                ...
                h->mHeapId, h->mBase, h->mSize);//STDLIBC_INTEGER
    }
}
