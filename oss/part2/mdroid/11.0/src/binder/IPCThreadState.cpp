...
//STDLIBC_INTEGER #include <binderthreadstate/IPCThreadStateBase.h>
...
//STDLIBC_INTEGER static std::atomic<bool> gHaveTLS(false);
static bool gHaveTLS = false;//STDLIBC_INTEGER

static pthread_key_t gTLS = 0;
//STDLIBC_INTEGER static std::atomic<bool> gShutdown = false;
static bool gShutdown = false;//STDLIBC_INTEGER
//STDLIBC_INTEGER static std::atomic<bool> gDisableBackgroundScheduling = false;
static bool gDisableBackgroundScheduling = false;//STDLIBC_INTEGER

IPCThreadState* IPCThreadState::self()
{
    //STDLIBC_INTEGER if (gHaveTLS.load(std::memory_order_acquire)) {
	if (gHaveTLS) {
restart:
    ...
    //STDLIBC_INTEGER  if (gShutdown.load(std::memory_order_relaxed)) {
	if (gShutdown) {
    ...
    //STDLIBC_INTEGER  if (!gHaveTLS.load(std::memory_order_relaxed)) {
	if (!gHaveTLS) { //STDLIBC_INTEGER
        ...
        //STDLIBC_INTEGER gHaveTLS.store(true, std::memory_order_release);
		gHaveTLS = true;//STDLIBC_INTEGER 
    }
    pthread_mutex_unlock(&gTLSMutex);
    goto restart;
}
...
{
    //STDLIBC_INTEGER gShutdown.store(true, std::memory_order_relaxed);
    gShutdown = true;//STDLIBC_INTEGER
    //STDLIBC_INTEGER  if (gHaveTLS.load(std::memory_order_acquire)) {
	if (gHaveTLS) { //STDLIBC_INTEGER
        ...
        //STDLIBC_INTEGER gHaveTLS.store(false, std::memory_order_release);
		gHaveTLS = false;//STDLIBC_INTEGER
    }
}
...
void IPCThreadState::disableBackgroundScheduling(bool disable)
{
   ...
   //STDLIBC_INTEGER gDisableBackgroundScheduling.store(disable, std::memory_order_relaxed);
}

bool IPCThreadState::backgroundSchedulingDisabled()
{
    //STDLIBC_INTEGER return gDisableBackgroundScheduling.load(std::memory_order_relaxed);
    ...
    if (result >= NO_ERROR) {
        ...
        if (mProcess->mExecutingThreadsCount < mProcess->mMaxThreads &&
            ...
            if (starvationTimeMs > 100) {
                ALOGE("binder thread pool (%zu threads) starved for %lld\" PRId64 \" ms",  //STDLIBC_INTEGER
