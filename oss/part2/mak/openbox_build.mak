...
LDFLAGS += -L${NCURSES_INSTALL_PATH}/lib
...
.PHONY: all bash coreutils findutils gawk grep gzip inetutils module_init_tools ncurses procps psmisc sed tar util-linux net-tools iputils udhcp

#all: bash coreutils findutils gawk gzip inetutils  ncurses procps sed tar util-linux  iputils udhcp grep net-tools module_init_tools  #psmisc
all: bash 
...
procps:
	...
	if [ "$(PROCPS_VER)" == "procps-ng-3.3.15"  ]; then \
		...
		cd ${PROCPS_SRC_BUILD_PATH}; \
		./configure --enable-shared=no --enable-skill ac_cv_func_malloc_0_nonnull=yes ac_cv_func_realloc_0_nonnull=yes --prefix=${PROCPS_INSTALL_PATH} --host=arm-linux CFLAGS="$(CFLAGS)" LDFLAGS="$(LDFLAGS)";\
		echo NCURSES_CFLAGS=$(NCURSES_CFLAGS) ;\
		make V=1 -C ${PROCPS_SRC_BUILD_PATH} ;\
		make -C ${PROCPS_SRC_BUILD_PATH} install;\
	fi
...
util-linux:
	cd ${UTIL_LINUX_SRC_BUILD_PATH}; \
	./autogen.sh; \
	./configure --prefix=${UTIL_LINUX_INSTALL_PATH} --host=arm-linux  --enable-static --enable-login-utils --without-ncurses --disable-use-tty-group \
	--without-python  --without-tinfo  --disable-bash-completion \
	--enable-static-programs=mount,umount,blkid
	make -C ${UTIL_LINUX_SRC_BUILD_PATH}
	make -C ${UTIL_LINUX_SRC_BUILD_PATH} install
